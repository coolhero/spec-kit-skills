...
Environment Setup (before first implement):
    +-- Aggregate required env vars from all active Features' pre-context.md
    +-- Check .env for missing variables
    +-- HARD STOP: Guide user to create/update .env file
    |   (User edits .env directly — secrets are never pasted into the conversation)
...
The state file includes Feature Progress, Feature Detail Log, Feature Mapping (Feature ID to spec-kit Name), Global Evolution Log, and Constitution Update Log. When `scope=core`, Features outside the Active Tiers are marked as `deferred` and skipped by the pipeline until activated via `/smart-sdd expand`.
...
| Environment setup checkpoint | Auto-aggregated with HARD STOP | You check `.env` yourself |
