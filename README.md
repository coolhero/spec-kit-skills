...
/smart-sdd init                          # Project setup → prompts to define Features via add
/smart-sdd init --prd path/to/prd.md     # PRD-based: extracts project meta, then Features via add
...
| Entry point | `/smart-sdd init` → `/smart-sdd add` | `/smart-sdd add` | `/reverse-spec` then pipeline |
...
| Feature granularity | Defined per Feature during add | N/A (adding to existing Features) | User selects from Coarse/Standard/Fine options during reverse-spec |
...
`/smart-sdd add` is the **universal Feature definition command** used by all modes (greenfield, incremental, rebuild). It guides the user through a structured 6-Phase consultation:

```
Phase 1: Feature Definition   — Adaptive consultation based on user readiness
Phase 2: Overlap & Impact     — Check against existing Features + constitution impact
Phase 3: Scope Negotiation    — Single vs split, Tier assignment (core scope)
Phase 4: SBI Match + Expand   — Map/create source behaviors (rebuild/adoption only)
...
|--------|---------|---------|
| `context-summary.sh` | Feature/Entity/API/DemoGroup summary | `add` Phase 2 |
| `sbi-coverage.sh` | SBI coverage dashboard with `--filter` | `add` Phase 4, verify post-check |
| `demo-status.sh` | Demo Group progress | `add` Phase 5, verify post-check |
