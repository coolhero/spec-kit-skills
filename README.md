...
Phase 3: Demo-Ready Verification (only if Demo-Ready Delivery is in the constitution) — BLOCKS on failure
    ...
    +-- Check two-mode support: --test (default) and --interactive (try it yourself)
    +-- Execute the demo script (--test mode) and verify it completes without errors
...
| **VI. Demo-Ready Delivery** | Each Feature must be demonstrable upon completion. "Tests pass" alone is NOT sufficient — provide an **executable demo script** (`demos/F00N-name.sh` or `.ts`/`.py`/etc.) with **two modes**: `--test` (default: automated assertions → passed/total → exit) and `--interactive` (keep Feature running so users can **try it themselves** with concrete copy-paste examples). The script **maps to spec.md FR-###/SC-###** and exercises as many functional requirements as possible. Demo code is categorized as **Demo-only** (`// @demo-only`, removed later) or **Promotable** (`// @demo-scaffold`, extended later) | `./demos/F00N-name.sh` verifies (test mode); `--interactive` lets users try the Feature |
