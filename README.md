...
[한국어 README](README.ko.md) | Last updated: 2026-03-05 03:00 KST
...
| **VI. Demo-Ready Delivery** | Each Feature must be demonstrable upon completion. "Tests pass" alone is NOT sufficient — provide an **executable demo script** (`demos/F00N-name.sh` or `.ts`/`.py`/etc.) that **launches the real, working Feature** so the user can experience it firsthand. The script starts the service, prints concrete "Try it" instructions (URLs, curl commands), and keeps running until Ctrl+C. `--ci` flag for automated health check (used by `verify`). Coverage header **maps to spec.md FR-###/SC-###** showing what the user can try. Demo code is categorized as **Demo-only** (`// @demo-only`, removed later) or **Promotable** (`// @demo-scaffold`, extended later). **Demo task injection**: tasks.md is checked for demo tasks; if missing, a warning recommends adding them. **Limited verification**: if verify Phase 1/3 can't pass (external dependencies), user may acknowledge ⚠️ limited-verify with a reason — merge is allowed with reminder | `./demos/F00N-name.sh` launches the Feature — user can see, use, and interact with it |
