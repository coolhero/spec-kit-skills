...
#### L73. Runtime Environment Is a First-Class Toolchain Concern

**What happened**: aegis demo scripts hardcoded `npm run start:dev`, `curl localhost:3000/health`, and assumed `.env` would be available in shell environment. When the project's `.env` loading mechanism (NestJS @nestjs/config) didn't apply to bash scripts, demos failed silently. The same server startup and health check patterns were duplicated in demo scripts, verify Phase 3, and implement smoke — each with slightly different implementations.

**Universal takeaway**: Build toolchain (F8: build, test, lint) and runtime toolchain (F8b: server_start, health_check, env_loading, prerequisites) are equally important for pipeline automation. F8b centralizes runtime environment knowledge so that demo, verify, and implement all read from the same source. When the Foundation changes (NestJS → FastAPI), F8b changes once and all consumers adapt. Without F8b, each consumer hardcodes framework-specific commands and diverges over time.

#### L74. Integration Demo Repetition Catches State Pollution Bugs
...
#### L79. Exploration Has Two Purposes — Discovery and Understanding

**What happened**: code-explore was designed for pre-pipeline discovery: scan a codebase, generate Feature candidates, hand off to smart-sdd. But users also wanted to run it on projects that were already fully SDD-managed — not to generate new Features, but to understand how existing Features work. Running default explore on a completed project produced Feature candidates that duplicated existing Features, creating confusion about whether these were new work items or just the explore tool not knowing about existing state.

**Universal takeaway**: Tools that serve a pipeline phase (discovery) often get used outside that phase (understanding). When a tool has two distinct purposes, make them explicit modes rather than hoping one flow handles both. Indicators you need a mode split: (1) users run the tool at a lifecycle stage it wasn't designed for, (2) the tool produces output that conflicts with existing state, (3) users want the tool's analysis but not its generative output. The fix is a dedicated mode (`--learn`) that shares the analysis engine but suppresses generative output.

#### L80. Gates That Aren't in SKILL.md Don't Exist
