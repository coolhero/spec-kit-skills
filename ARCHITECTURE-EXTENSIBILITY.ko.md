...
spec-kit-skills는 규모 있는 Agentic Coding의 세 가지 구조적 gap을 해결하는 세 가지 설계 원칙 위에 구축되었습니다. 이 원칙들은 네 개의 Claude Code 스킬(code-explore, reverse-spec, smart-sdd, domain-extend)을 통해 구현됩니다:
...
| **Domain Profile** | 에이전트가 프로젝트 유형에 관계없이 동일한 범용 접근을 적용 | 합성형 5축 시스템(Interface × Concern × Archetype × Foundation × Context)으로 프로젝트 유형별 규칙 로드. 각 프로젝트는 sdd-state.md + domain-profile-instance.md에 저장되는 구체적인 Instance를 가짐 |
...
8. [새 Context Mode 추가](#8-새-context-mode-추가)
...
6. contexts/modes/{mode}.md    (하나의 컨텍스트 모드)
6b. contexts/modifiers/*.md   (활성 수정자, 예: migration)
...
**3단계 모듈 해석(Three-tier module resolution)**: Resolver는 세 위치에서 순서대로 모듈을 스캔합니다:

1. **Built-in** (스킬 설치 디렉토리) — spec-kit-skills에 기본 탑재된 47개 concern, 15개 archetype, 40개 이상의 foundation. lazy loading을 위해 shared/reverse-spec/smart-sdd 디렉토리에 분산.
2. **Project** (대상 프로젝트의 `specs/domains/`) — `/domain-extend`로 생성한 커스텀 모듈. 단일 파일 형식(모든 S/A/R/F 섹션을 하나의 파일에 포함)으로, git에 커밋되고 프로젝트별로 격리됨. Resolver가 Step 6b에서 이 디렉토리를 스캔.
3. **Org** (`org-convention.md`) — 모든 프로젝트에 적용되는 조직 전역 공유 규칙.

`/domain-extend extend`로 프로젝트 로컬 모듈을 생성(기본값)하거나, `/domain-extend extend --skill`로 공유 spec-kit-skills 설치에 built-in 모듈을 기여할 수 있습니다.
...
도메인 합성 시스템은 규칙을 생산하고 깊이를 조절하는 **5개 축**을 가집니다. 각 구성요소는 다른 질문에 답합니다:
...
```
  ...
  ARCHETYPE ────────┤  어떤 도메인 철학인가?                │──── ai-assistant, public-api, microservice
                    │                                     │
  FOUNDATION ───────┤  어떤 프레임워크 제약인가?             │──── React, Electron, Next.js (40+)
                    │                                     │
  CONTEXT ─────────┤  프로젝트 상황은?                     │──── Mode: greenfield/rebuild/incremental/adoption
                    │                                     │     Scale: prototype/mvp/production × solo/small-team/large-team
                    │                                     │     Modifiers: +migration, +compliance, ...
                    └─────────────────────────────────────┘
```

### 축 1-4 vs 축 5 (Context)

- 축 1-4는 **도메인 규칙**을 생산합니다: "IPC 호출에 timeout handling SC가 필수"
- Context의 **mode**는 파이프라인 동작을 선택합니다: rebuild는 보존 규칙, 마이그레이션 게이트를 추가
- Context의 **scale**은 깊이를 조절합니다: "prototype 모드에서 timeout SC는 선택"
- Context의 **modifiers**는 상황별 오버레이를 추가합니다: +migration이 M0-M4 프레임워크를 활성화
...
| 5 | **Context** | _상황_ — 모드, 스케일, 수정자 | Rebuild 모드는 보존 규칙을 정의; production 스케일은 완전한 커버리지를 강제; +migration은 M0-M4 게이트를 추가 |
...
Context:     rebuild / mvp / solo     ← 축 5 (모드 / 스케일)
```

6개 도메인 모듈 + 2개 Foundation 파일 = 8개 파일 로드, 모두 세션 동안 캐시. Context scale이 조절: MVP는 핵심 경로의 SC 커버리지는 완전하지만 엣지 케이스는 선택적; solo는 코드 리뷰 요구사항 없음.
...
**5축 + Modifier → 5축 (Context 통합)**: 분리되어 있던 Scenario 축과 Scale modifier를 단일 **Context** 축으로 통합했습니다. Mode(파이프라인 구조), Scale(깊이 조절), Modifiers(+migration 등 상황별 오버레이)의 세 하위 구성요소를 가집니다. "5개 축 + 1 modifier"의 비대칭을 제거하고, 횡단적 상황 관심사를 위한 자연스러운 위치를 제공합니다.
...
```
로드 순서: _core → interfaces → concerns → archetypes → foundations → org-convention → context modes → context modifiers → custom
...
병합 결과:  파일이 아님 — 에이전트 메모리의 행동 규칙셋
              ...
              + Foundation    + Context 조절  + Foundation
                (F2,F7)                        (F8)
                    │               │               │
                    ▼               ▼               ▼
              형성            형성            형성
...
에이전트는 합성 산출물을 "생성"하지 않습니다. 병합된 규칙이 각 단계에서 **다르게 동작**하도록 지시하기 때문입니다: "SC 작성 시 IPC 패턴도 검사하라" 또는 "verify 시 Playwright UI 테스트도 실행하라." Context scale이 이후 조절합니다: `prototype` 모드에서 비핵심 검사는 선택적이 되고, `production` 모드에서 모든 검사가 차단 조건이 됩니다. 모듈은 보이지 않는 인프라입니다 — 사용자는 더 나은 스펙, 더 철저한 계획, 더 신뢰할 수 있는 구현을 경험합니다.
...
기존 인터페이스(gui, http-api, cli, data-io, tui, mobile, library, embedded, grpc, k8s-api)로 커버되지 않는 별도의 인터랙션 표면이 있을 때 추가합니다.
...
| `cqrs-eventsourcing` | CQRS / Event Sourcing 패턴 | Command/Event 흐름, 프로젝션, 사가 오케스트레이션, 이벤트 리플레이 |
| `dag-orchestration` | DAG 기반 워크플로우 오케스트레이션 | Airflow, Prefect, Dagster, dbt, 태스크 의존성, 백필 |
| `distributed-consensus` | 합의 프로토콜과 분산 합의 | Raft, Paxos, 가십, 리더 선출, 로그 복제 |
| `ecs` | Entity-Component-System 아키텍처 | 데이터 지향 설계, 컴포넌트 쿼리, 시스템 스케줄링 |
| `hardware-io` | 하드웨어 I/O 및 디바이스 통신 | Serial, USB, GPIO, SPI, I2C, 디바이스 생명주기, HAL |
| `k8s-operator` | Kubernetes 오퍼레이터 패턴 | CRD, 재조정 루프, 파이널라이저, 상태 서브리소스 |
| `wire-protocol` | 와이어 프로토콜 구현 | MQTT, AMQP, RESP, 프레임 파싱, 프로토콜 상태 머신 |
...
| `browser-extension` | 브라우저 확장 프로그램 및 애드온 | Minimal Permissions, Content Script Isolation, Background Lifecycle, Storage-First, Cross-Browser |
| `database-engine` | 스토리지 엔진, 쿼리 프로세서, 데이터베이스 시스템 | Storage/Compute Separation, ACID, Query Lifecycle, Replication, Schema Evolution |
| `game-engine` | 게임 엔진 및 인터랙티브 시뮬레이션 | Deterministic Update Loop, Data-Oriented Design, Render-Update Separation, Asset Lifecycle, Frame Budget |
| `infra-tool` | Infrastructure-as-Code 도구 및 재조정 엔진 | Declarative Over Imperative, Provider Extensibility, State as Source of Truth, Idempotent, Plan Before Apply |
| `message-broker` | 메시지 브로커 및 이벤트 스트리밍 플랫폼 | Protocol Conformance, Message Ordering, Durability/Throughput Tradeoff, Cluster Consensus, Backpressure |
| `microservice` | 분산 서비스 아키텍처 | Service Autonomy, Contract-First, Circuit Breaking, Observability |
| `network-server` | 프록시, 로드 밸런서, API 게이트웨이 | Connection Lifecycle, Filter Chain, Protocol State Machine, Hot-Reload, Backpressure |
...
| **Python** (언어) | (언어 수준 규칙) | `python.md` | Compact |
| **Go** (언어) | (언어 수준 규칙) | `go.md` | Compact |
| **Rust** (언어) | (언어 수준 규칙) | `rust-cargo.md` | Compact |
| **Erlang/OTP** (언어) | (언어 수준 규칙) | `erlang-otp.md` | Compact |
...
# Context mode는 sdd-state.md의 Origin 필드로 결정되며, profile이 아닙니다.
```
...
Context mode는 프로젝트가 _왜_ 만들어지는지에 따른 파이프라인 동작 변형을 정의합니다.

### 추가 시점

드뭅니다. 4가지 컨텍스트 모드 (greenfield, rebuild, incremental, adoption)가 대부분의 사용 사례를 커버합니다. 새 프로젝트 상황에 파이프라인이 근본적으로 다른 동작을 필요로 할 때만 추가합니다.

### 절차

1. `.claude/skills/smart-sdd/domains/contexts/modes/{name}.md` 생성
2. 컨텍스트 모드가 기여하는 S-섹션 정의 (일반적으로 S1, S3, S5, S7)
3. 컨텍스트 모드에 특수 해결 규칙이 있으면 `_resolver.md` 업데이트
...
| **Level 2** | 🔶 ~60% | Cross-Concern Integration Rules가 `_resolver.md` Step 3.5에 정의됨 (65개 규칙). 최근 `context-injection-rules.md` 및 모든 injection 파일에 enforcement 연결 완료. Context scale enforcement 추가됨. 충돌 해결 규칙은 문서화된 쌍에 한정 |
...
| **Level 5** | ⬜ ~15% | skill-feedback.md 접수 프로세스 존재; 70개 이상 SKF 항목 처리 완료. 규칙 효과성 자동 추적은 아직 미구현 |
...
3. 이제 파이프라인이 로드: `_core → http-api → auth → public-api → contexts/modes/greenfield`
...
| **S4 데이터 무결성** | `smart-sdd/domains/_core.md` § S4a-S4c (보편), `smart-sdd/domains/contexts/modes/rebuild.md` § S4d (rebuild 전용), `smart-sdd/domains/concerns/ipc.md` § S7 IPC N-Layer (S4 확장), `smart-sdd/domains/_schema.md` § S4 (스키마 정의) |
...
| **spec-draft.md** | `reverse-spec/templates/spec-draft-template.md` (형식), `reverse-spec/commands/analyze.md` § Phase 4 spec-draft 생성 (변환 규칙), `smart-sdd/reference/injection/specify.md` § Spec-Draft Seeding Protocol (소비), `smart-sdd/reference/context-injection-rules.md` § Missing/Sparse (퇴보) |
| **UI Flow Spec** | `reverse-spec/commands/analyze.md` § Phase 1.5-D (생성), `reverse-spec/templates/pre-context-template.md` § UI Flow Specifications (저장), `smart-sdd/reference/injection/specify.md` § Read Targets (소비) |
| **Artifact Separation** | `CLAUDE.md` § P1-b (원칙), `reverse-spec/` (소스 분석 아티펙트), `specs/NNN-feature/` (파이프라인 아티펙트 — 소스 참조 없음) |
| **Cross-Concern Integration** | `smart-sdd/domains/_resolver.md` § Step 3.5 (규칙 테이블), `ARCHITECTURE-EXTENSIBILITY.md` § 2 (패턴 목록) |
| **Lazy Loading** | `smart-sdd/domains/_resolver.md` § Step 5 (명령별 섹션 필터링) |
| **Wiring Check** | `smart-sdd/reference/injection/implement.md` § App Lifecycle Wiring Check (7-point 게이트), `smart-sdd/scripts/wiring-check.sh` (자동화) |
| **SC Evidence Gate** | `smart-sdd/commands/verify-evidence-update.md` § SC Verification Evidence Gate (차단), `smart-sdd/commands/verify-sc-verification.md` § Step 0 SC Verification Planning |
...
| **Foundation 파일 (rust)** | `reverse-spec/domains/foundations/rust-cargo.md` (전체 모듈) |
...
| **Foundation 파일 (멀티 플랫폼, 전체)** | `reverse-spec/domains/foundations/{python,go,erlang-otp}.md` (전체 모듈) |
| **Foundation 파일 (멀티 플랫폼, 스텁)** | `reverse-spec/domains/foundations/{swift-spm,nuxt,angular,remix,qt,gtk,symfony,wordpress,android-native}.md` (감지 스텁) |
...
| **Migration context (shared)** | `shared/domains/contexts/modifiers/migration.md` — M0 시그널 감지, M1 규모 분류 (Hotfix→Platform), M2 대상 계층, M3 영향 평가, M4 파이프라인 깊이 조절 |
| **Migration context (reverse-spec)** | `reverse-spec/domains/contexts/modifiers/migration.md` — R3 마이그레이션 유형별 Feature 경계, R4 의존성/데이터 흐름 추출, R5 범위 추정 |
