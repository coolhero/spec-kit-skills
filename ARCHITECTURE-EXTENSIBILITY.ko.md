...
파이프라인 아티펙트(`spec.md`, `plan.md`, `tasks.md`)는 **요구사항만** 포함 — 소스 코드 참조 없음. **아티펙트 분리**로 spec 재사용성 + 소스 분석 단일 출처 보장.

```
소스 코드 → reverse-spec  → specs/reverse-spec/    → smart-sdd  → specs/NNN-feature/
              (분석)          pre-context.md            (파이프라인)   spec.md (깨끗)
                              spec-draft.md (상세)                    plan.md (깨끗)
                              UI Flow Specs                           tasks.md (깨끗)
```
