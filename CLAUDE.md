...
- **Skill files** (SKILL.md, commands/*.md, reference/*.md, domains/*.md, templates/*.md) and **project files** (history.md, PLAYWRIGHT-GUIDE.md, lessons-learned.md) MUST be written in English. These are the framework's source code — language is not configurable.
- **Exception**: `README.ko.md` and `ARCHITECTURE-EXTENSIBILITY.ko.md` are the only skill files written in Korean.
...
- **case-study 스킬 (DEPRECATED)**: 파일(`/.claude/skills/case-study/`)은 존재하지만, **Auto-Report 시스템으로 대체됨** (`shared/reference/completion-report.md`). 보고서는 이제 reverse-spec Phase 4-5, smart-sdd adopt, smart-sdd pipeline 완료 시 자동 생성됨. case-study 파일은 하위 호환성을 위해 유지하되, README 소개 및 파일 테이블에서 **의도적으로 제외**. `case-study-log.md`는 더 이상 필요 없음 — 모든 정성적 데이터는 `history.md` per-Feature Implementation Decisions에 기록됨. README에 case-study를 다시 추가하지 마세요.
...
| `SCENARIO-CATALOG.md` | 시나리오 흐름 변경, 새 명령/모드 추가 → 관련 시나리오 업데이트 또는 신규 추가 |
