...
- **case-study 스킬**: 파일(`/.claude/skills/case-study/`)은 존재하지만, README 소개 및 파일 테이블에서 **의도적으로 제외**됨. reverse-spec과 smart-sdd가 case-study-log 템플릿/프로토콜을 참조하므로 코드는 삭제하지 않지만, 독립 스킬로 소개하지 않음. README에 case-study를 다시 추가하지 마세요.
