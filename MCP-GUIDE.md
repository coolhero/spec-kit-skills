...
### 방법 C: Web Preview Mode (재시작 불필요, `/reverse-spec` Phase 1.5 기본)

Electron의 **renderer dev server만** 독립 실행하고 Playwright가 일반 브라우저로 접속합니다. MCP 재설정이나 Claude Code 재시작이 필요 없습니다.

> **장점**: CDP 설정 변경 → 재시작 → 세션 초기화 문제를 완전히 회피합니다. `/reverse-spec` Phase 1.5에서 CDP가 미설정된 경우 자동으로 이 모드를 사용합니다.

**원리**: 대부분의 Electron 앱은 renderer가 독립적인 웹앱(Vite, Webpack 등)입니다. renderer dev server만 따로 실행하면 동일한 웹 UI를 일반 브라우저에서 볼 수 있습니다.

**제한사항**: Electron 전용 기능(네이티브 메뉴, IPC, 시스템 트레이, 파일 시스템 접근)은 보이지 않습니다. UI 레이아웃과 사용자 흐름 파악에는 충분합니다.

| 빌드 도구 | Renderer Dev Server 명령 | 기본 포트 |
|-----------|-------------------------|----------|
| **electron-vite** | `npx vite --root src/renderer` | 5173 |
| **별도 스크립트 있는 경우** | `npm run dev:web` (또는 `dev:renderer`) | 스크립트에 따라 다름 |
| **electron-forge + webpack** | `npx webpack serve --config webpack.renderer.config.js` | 8080 |

```bash
# electron-vite 예시
cd /path/to/electron-project
npx vite --root src/renderer
# → http://localhost:5173 에서 renderer UI 접속 가능
```

> **방법 선택 가이드**: Web Preview (방법 C) → 빠르고 재시작 불필요, UI 탐색 목적에 충분. CDP (방법 B) → Electron 전용 기능까지 검증 필요 시. `--electron-app` (방법 A) → 공식 지원 안정화 후 사용.

### Runtime Exploration 시 CDP 전환 (방법 B 사용 시)

> **Note**: `/reverse-spec` Phase 1.5는 기본적으로 **방법 C (Web Preview Mode)**를 사용합니다. 아래 CDP 전환은 방법 B를 명시적으로 선택한 경우에만 필요합니다.

Playwright MCP에 `--cdp-endpoint`를 추가해야 하며, 이 설정은 **일반 웹 탐색과 양립 불가**합니다. 따라서 Electron 탐색 전후로 전환이 필요합니다.
