...
`claude mcp get playwright`에서 `Status: ✓ Connected`는 **MCP 서버 프로세스가 실행 중**이라는 의미입니다.
Playwright가 **브라우저에 연결 성공**했다는 의미가 **아닙니다**.

| 표시 | 실제 의미 | 확인 방법 |
|------|----------|----------|
| `Status: ✓ Connected` | MCP 서버 프로세스 실행 중 | `claude mcp get playwright` |
| 브라우저 연결 성공 | Playwright 도구가 세션에 로드됨 | Claude Code 세션에서 `browser_snapshot` 호출 가능 여부 |

CDP 모드에서 "Connected"이지만 도구가 사용 불가한 경우:
- CDP endpoint에 앱이 실행되지 않고 있음 → 앱 실행 후 세션 재시작
- 세션 시작 후 앱을 실행함 → 세션 재시작 필요 (도구는 세션 시작 시 로드)
...
Electron + CDP 방식은 다음 순서를 **반드시** 준수해야 합니다:

```text
1. npm run build                              ← 앱 빌드
2. npx electron out/main/index.js \
     --remote-debugging-port=9222             ← CDP 포트로 앱 실행
3. curl localhost:9222/json/version           ← CDP 연결 확인
4. Claude Code 세션 시작 (또는 재시작)         ← MCP가 CDP 연결 → 도구 로드
```

**순서가 중요한 이유**: Claude Code 세션 시작 시점에 MCP가 도구를 로드합니다.
세션 시작 후 앱을 실행하면 CDP 포트가 없으므로 도구가 로드되지 않습니다.
세션 시작 후 앱을 실행한 경우, **세션을 재시작**해야 도구가 정상 로드됩니다.
...
| `claude mcp get` Connected인데 도구 없음 | MCP 프로세스 ✓, 브라우저 연결 ✗ | `curl localhost:9222/json/version` 확인 |
| curl 성공인데 도구 없음 | 세션 시작 후 앱 실행 | 세션 재시작 필요 |
| browser_snapshot → 일반 Chrome 탭 | CDP 미설정 (standard mode) | `--cdp-endpoint` 추가 후 재등록 |
| browser_snapshot → connection refused | CDP 설정 ✓, 앱 미실행 | 앱 실행 후 재시도 |
