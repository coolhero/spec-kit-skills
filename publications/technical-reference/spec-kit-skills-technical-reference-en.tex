...
The real power emerges when modules \emph{combine}. The resolver detects specific combinations and activates Cross-Concern Integration Rules---65 combination patterns defined in \file{\_resolver.md}. When a project includes both \texttt{gui} and \texttt{realtime}, the resolver triggers emergent rules that neither module alone contains: optimistic update patterns, reconnection UI states, and ``stale UI after reconnect'' prevention.
...
This principle enables the Context Reset Protocol---clearing context between work units is safe because all state is in files. See Chapter~\ref{ch:advanced} \S~Context Reset Protocol for the full procedure.
...
\subsection{Seven Evasion Patterns}
...
\begin{warnbox}[P2 Corollary: Line Position Matters (L88)]
P2 applies not only \emph{across} files (reference vs.\ inline) but \emph{within} files. A rule at line~1453 of a 2000-line file has near-zero compliance probability---the agent reads the first 800--1000 lines and stops. During the aegis pilot, Per-Task Micro-Verify was defined at \texttt{pipeline.md:1453} and \texttt{injection/implement.md:598}. The agent never reached either line. Result: 40+ files written without a single runtime check.

\textbf{Design rule}: critical enforcement rules must appear in the first 100 lines of the file the agent actually loads (SKILL.md), not buried deep in a 2000-line reference file. The reference file provides detail; SKILL.md provides the trigger.
\end{warnbox}
...
The project includes 20 gap patterns and 65 specific lessons, each traced to a real incident. This is not theoretical---every countermeasure was motivated by an observed failure. The catalog itself is a contribution: these patterns apply to anyone building AI agent workflows, regardless of tooling.

\subsection{Engineering Depth}

The module system (47 concerns, 15 archetypes, 10 interfaces, 40+ foundations, 65 cross-concern integration rules) represents genuine domain engineering. Each module encodes expert knowledge about what matters for a specific project type---knowledge that would otherwise exist only in senior engineers' heads.
...
When multiple concerns are active, emergent patterns arise that neither module produces alone. These 65 rules are defined in \file{\_resolver.md} Step~3.5:
...
\section{F-Family: Foundation Platform (F0--F8b)}
...
F8b & Runtime Environment & Server startup, health check, environment loading for demos and verify \\
...
Runtime fields for demo scripts, verify Phase~3, and implement smoke launch. Centralizes server startup, health checks, and environment loading so all consumers read from one source.
...
\midrule
server\_start & Development server start command \\
server\_port & Default port number \\
health\_check & Health check command (exit 0 = healthy) \\
env\_loading & How framework loads env vars (dotenv, python-dotenv, shell-native) \\
prerequisites & Infrastructure start (docker compose up -d) \\
seed\_data & Test data seeding command \\
cleanup & Infrastructure teardown \\
\bottomrule
\end{tabularx}
\end{table}

Consumers: demo scripts read F8b for server startup; verify Phase~3 reads F8b before SC runtime verification; implement smoke launch reads F8b for health checks. When Foundation changes (NestJS $\rightarrow$ FastAPI), F8b changes once and all consumers adapt.
...
The previous sections showed how four families define rules (\S5.1--5.4), merge at each pipeline stage (\S5.6), and produce rich SC coverage through accumulation (\S5.5). But how does this play out in real-world projects? The following migration scenarios demonstrate all four families working together across different dimensions of change---database, architecture, interface, and runtime. Each scenario shows which families activate, what rules they contribute, and how the \texttt{+migration} context modifier adds behavioral parity requirements.
...
For detailed walkthroughs of all six scenarios showing which families activate in each, see Chapter~\ref{ch:scenarios} \S~Complete Scenario Reference.
...
\begin{tipbox}[Chapter 5 Key Takeaways]
\begin{itemize}[nosep]
  \item Four section families (S/A/R/F) separate pipeline execution, architecture philosophy, code analysis, and framework platform rules
  \item Each section maps to exactly one pipeline stage---enabling lazy loading (55--70\% context savings)
  \item Families merge by accumulation, not override---all relevant rules stack
  \item Six project scenarios activate different family combinations
\end{itemize}
\end{tipbox}

The Section System defines \emph{what knowledge exists} inside each module. The next chapter explains \emph{how that knowledge reaches the agent}---the loading order, injection protocol, context budget management, and merge semantics that compose hundreds of module sections into a focused, per-step context.
...
\subsection{Concrete Loading Trace}

For a project with Domain Profile: \texttt{gui + auth + realtime + ai-assistant + electron + rebuild}:

\begin{lstlisting}[title={Module Loading Trace (specify step)}]
Step 1: _core.md S1          -> 12 universal SC rules loaded
Step 2: gui.md S1            -> "Every form needs validation SC" + 8 rules
Step 2: http-api.md S1       -> (not loaded -- gui project, not http-api)
Step 3: auth.md S1           -> "Login/logout/token SC required" + 6 rules
Step 3: realtime.md S1       -> "Streaming lifecycle SC required" + 5 rules
Step 4: ai-assistant.md A2   -> "Provider abstraction SC" + 4 extensions
Step 4b: electron.md F2      -> "IPC bridge, main/renderer SC" + 3 constraints
Step 5: (no org-convention)  -> skipped
Step 6: rebuild.md           -> "Source parity SC required"
Step 6b: specs/domains/      -> (no project-local modules)
Step 7: (no domain-custom)   -> skipped

Result: 38 rules from 6 modules + 1 context mode
        (vs. 200+ rules if ALL modules loaded = 81% context savings)
\end{lstlisting}

Compare with a CLI project (\texttt{cli + resilience + microservice + go}): completely different modules load, producing completely different rules. Same pipeline, different domain knowledge.
...
\section{Rule Extensibility: Principles vs.\ Tools}
\label{sec:rule-extensibility}

A critical design constraint: universal rules (in \file{\_core.md}, \file{\_foundation-core.md}, \file{verify-build-test.md}) must express \textbf{principles}, not \textbf{tools}. Framework-specific tools belong in individual Foundation files.

\begin{itemize}[nosep]
  \item \textbf{Principle} (universal): ``Foundation Feature must configure a linter---installed without config is incomplete.''
  \item \textbf{Tool} (framework-specific): ``ESLint v10 requires \texttt{eslint.config.js} flat config format.''
\end{itemize}

The principle lives in \file{\_foundation-core.md} (applies to all projects). The tool lives in \file{foundations/nestjs.md} F8 section (applies only to NestJS projects). This separation ensures:

\begin{enumerate}[nosep]
  \item A Python project using \texttt{ruff} isn't told to create \texttt{eslint.config.js}
  \item A Go project isn't warned about missing \texttt{.prettierrc}
  \item New frameworks can be added without modifying universal rules
\end{enumerate}

\begin{warnbox}[Common Violation: Tool Names in Universal Rules]
During the aegis pilot, \file{\_foundation-core.md} contained ``\texttt{npx tsc --noEmit} exits 0'' as a universal BST check. This is wrong for Python (\texttt{mypy}), Go (\texttt{go vet}), or Rust (\texttt{cargo check}) projects. The fix: universal rule says ``type check command exits 0''; the Foundation file's F8 section defines which command.

\textbf{Test}: If a rule mentions a specific tool name (npm, eslint, prettier, jest) outside of an ``e.g.'' context, it likely needs generalization.
\end{warnbox}

Module loading ensures the right rules reach the right pipeline step. But what if the agent ignores those rules, or the pipeline proceeds with stale data? The next chapter describes seven structural guards that enforce correctness at stage boundaries---the safety net beneath the module system.
...
\subsection{How Guards Interact: A Rebuild Case Study}
...
Guards protect individual pipeline steps. But information must also survive \emph{across} Features---Entity~1's data model must be available when Feature~3 runs specify. The Global Evolution Layer provides this cross-Feature memory.
...
\begin{description}[style=nextline]
  ...
  \item[\file{constitution-seed.md}] Draft principles from \cmd{init} or \cmd{reverse-spec}---the input to constitution finalization. Version-controlled (unlike \file{.specify/memory/constitution.md} which is spec-kit's internal format). Edit the seed, re-run constitution to propagate.
  \item[\file{coverage-baseline.md}] SBI coverage tracking for rebuild/adoption projects. Maps source behaviors to Feature implementations with P1/P2/P3 priorities. Records intentional exclusions with rationale. Generated by reverse-spec Phase~4.
  \item[\file{verify-report.md}] Per-Feature runtime verification evidence. SC-by-SC pass/fail with Method (runtime/RUNTIME\_BLOCKED/RUNTIME\_DELEGATED), Phase~1--4 results, bugs found, and merge readiness decision. BLOCKING for merge gate. Located at \file{specs/NNN-feature/verify-report.md}.
  \item[\file{demos/F00N-name.sh}] Per-Feature executable demo script. Default mode starts server with instructions; \texttt{--ci} mode runs automated checks. Created during implement, verified during verify Phase~3.
  \item[\file{demos/DG\{N\}-scenario.sh}] Integration Demo script for Demo Group completion. Cross-Feature user journey verification. Created during last Feature's implement, executed at verify Phase~5. Same dual-mode structure as Feature demos.
\end{description}
...
The four registries work as a coordinated memory system. The following examples show what each registry actually contains---not just structure, but the reasoning behind each entry. Notice how entities record their owner Feature and consumers, enabling Feature~4 to discover that \texttt{User.orgId} already exists (decided in Feature~3) rather than reinventing it.
...
\begin{tipbox}[Chapter 8 Key Takeaways]
\begin{itemize}[nosep]
  \item Four registries: entity-registry (data), api-registry (contracts), sdd-state (progress), domain-profile-instance (decisions)
  \item Each Feature reads registries at specify, writes at plan Post-Update
  \item Cross-Feature consistency is structural (registries), not memory-based (agent recall)
  \item verify-report.md is a GEL artifact---persistent evidence, not chat-level claims
\end{itemize}
\end{tipbox}

Part~II described the architecture: Domain Profile shapes rules, modules load selectively, guards enforce correctness, and GEL preserves cross-Feature memory. Part~III now shows how the four skills use this architecture. \texttt{code-explore} is the entry point---it generates the orientation artifacts and Feature candidates that \texttt{reverse-spec} refines and \texttt{smart-sdd} consumes. \texttt{domain-extend} grows the vocabulary that all three draw from. Each skill reads from and writes to the same GEL artifacts described in Chapter~8.
...
\section{Learn Mode (\texttt{--learn})}

For projects that are already implemented and SDD-managed, \texttt{--learn} provides read-only exploration without generating new Feature candidates or conflicting with existing pipeline state.

\begin{description}
  \item[Purpose] Understand an existing project's architecture and code structure
  \item[Output] \file{specs/explore/learn/synthesis.md} --- architecture summary + existing Feature map
  \item[Does NOT produce] Feature candidates (C001\ldots), Domain Profile inference, handoff prep
  \item[Implies] \texttt{--no-branch} (no git branch created)
  \item[Use cases] Team onboarding, architecture review, audit, pre-meeting preparation
\end{description}

\begin{tipbox}[Default vs Learn Mode]
Default mode asks: ``What Features should we build?'' Learn mode asks: ``How does what we already built work?'' Use default before the pipeline; use \texttt{--learn} after.
\end{tipbox}
...
Eight mandatory rules are always loaded in SKILL.md:
...
\textbf{Rule~4}: Features execute sequentially. Never process multiple Features in parallel.

\textbf{Rule~5}: Runtime SC verification required. Build+test passing is NOT verify completion.

\textbf{Rule~6}: Honest SC evidence. Partial verification = PARTIAL, not PASS. User Demo required.

\textbf{Rule~7}: Implement Completeness Gate. Before implement Review: all tasks done + demo script exists + smoke launch passes.

\textbf{Rule~8}: No Code Without Spec. When verify discovers missing functionality, update spec.md FIRST (Cascading Update Protocol), then implement. Never write code that has no FR/SC in the spec.
\end{dangerbox}
...
The six phases are:
\begin{enumerate}[nosep]
  \item \textbf{Phase 1: Briefing}---parse input, identify perspective gaps, elaborate via S5/A3 probes
  \item \textbf{Phase 2: Overlap \& Impact}---check for overlap with existing Features, assess GEL impact
  \item \textbf{Phase 3: Scope Negotiation}---resolve ambiguities, confirm In-Scope/Out-of-Scope boundaries
  \item \textbf{Phase 4: SBI Match}---map to Source Behavior Inventory (brownfield), confirm coverage
  \item \textbf{Phase 5: Demo Group}---assign to Demo Group for Integration Demo coordination
  \item \textbf{Phase 6: Finalization}---generate artifacts (pre-context, sdd-state update, add-draft)
\end{enumerate}

\section{The 6-Phase Verify (Phase 0--5)}
...
\begin{description}
  ...
  \item[Phase 3: UI/Runtime Verification] Launch application via Playwright. Verify each SC against running UI. Delegate to user if Playwright unavailable---never skip. (Server startup uses F8b Runtime Environment fields from the active Foundation---see Chapter~5 \S~F8b.)
  \item[Phase 4: Cross-Feature Integration] Verify Features work together.
  \item[Phase 5: Integration Demo (conditional)] If this Feature completes a Demo Group, run the Integration Demo script. Skipped if no DG is completed.
\end{description}
...
The most complex pipeline step, split into 6 phases (Phase 0--5, described in Section~\ref{sec:4-phase-verify}).

\subsection{verify-report Quality Standards}

After verify completes, the \file{verify-report.md} artifact must meet three quality standards. These are not procedural instructions (how to run verify) but \textbf{output-format constraints} (how to structure the report so it serves as auditable evidence). They were added after the aegis pilot revealed that reports could technically exist but contain misleading data---``unit test'' in the Method column, lint silently omitted, test count not tracked.

The verify-report is not just a checklist---it is \textbf{auditable evidence}. Three quality standards were added based on the aegis pilot:

\paragraph{1. Method Column --- Closed Value Set.}
Each SC's Method must be exactly one of:
\begin{itemize}[nosep]
  \item \texttt{runtime: [specific method]}---verified against running application
  \item \texttt{RUNTIME\_BLOCKED: [reason]}---cannot verify at runtime, reported to user
  \item \texttt{RUNTIME\_DELEGATED: [reason]}---delegated to user for manual verification
\end{itemize}

``Unit test,'' ``code review,'' or ``by design'' are \textbf{not valid} Method values. If an SC can genuinely only be tested via unit test (e.g., environment validation that requires process restart), use \texttt{RUNTIME\_BLOCKED} with explanation.

\paragraph{2. Lint Status --- Foundation Awareness.}
If \texttt{npm run lint} fails with ``not installed'':
\begin{itemize}[nosep]
  \item For F001 (Foundation): this is a \textbf{spec gap}---lint should be part of Foundation setup
  \item For F002+: report as warning, not blocking (Foundation didn't set it up)
\end{itemize}

Either way, lint status \textbf{must appear} in the report. Silent omission is not allowed.

\paragraph{3. Test Growth Tracking.}
Each Feature's verify-report compares its test count against the preceding Feature:
\begin{itemize}[nosep]
  \item Test count increased: \texttt{37 -> 52 (+15 auth/RBAC tests)}---healthy
  \item Test count unchanged: \texttt{37 -> 37 (no new tests)}---warning: new code without regression tests
\end{itemize}

This is a warning, not a blocker. Runtime SC verification confirms the Feature works \emph{now}; test growth confirms it will \emph{keep working} after future changes.

\subsection{RUNTIME\_BLOCKED: When SCs Cannot Be Verified at Runtime}

Not every SC can be verified against a running application. Some require infrastructure, external services, or conditions that aren't available during development verification. The \texttt{RUNTIME\_BLOCKED} status exists for these cases---it is \textbf{honest acknowledgment}, not a shortcut.

\paragraph{The Three Valid SC Methods.}

\begin{description}
  \item[\texttt{runtime: [method]}] SC verified against the running application. This is the gold standard. Example: \texttt{runtime: curl POST /auth/login → 200 with token}
  \item[\texttt{RUNTIME\_BLOCKED: [reason]}] SC cannot be verified at runtime. The reason must be specific and structural---not ``too hard'' or ``takes too long.'' Example: \texttt{RUNTIME\_BLOCKED: requires 100 concurrent connections (load testing infrastructure)}
  \item[\texttt{RUNTIME\_DELEGATED: [reason]}] SC verification delegated to the user for manual confirmation. Used for OS-native interactions, visual judgments, or hardware-dependent behaviors. Example: \texttt{RUNTIME\_DELEGATED: drag-and-drop file upload requires manual browser interaction}
\end{description}

\paragraph{Common RUNTIME\_BLOCKED Categories.}

Based on the aegis pilot (F001--F004), these categories account for most RUNTIME\_BLOCKED SCs:

\begin{longtable}{p{3.5cm}p{4cm}p{5.5cm}}
\toprule
\textbf{Category} & \textbf{Why Blocked} & \textbf{Example from aegis} \\
\midrule
\endhead
\bottomrule
\endfoot

\textbf{Concurrency / Load} &
  Requires concurrent request generation infrastructure (e.g., 100 simultaneous requests) &
  SC-006: ``100 concurrent budget reservations must be atomic'' --- requires load testing tool (k6, Artillery) \\
\addlinespace

\textbf{External Webhook} &
  Requires an external HTTP endpoint to receive callbacks &
  SC-008/009: ``Alert webhook fires at 80\%/90\% threshold'' --- requires webhook receiver (e.g., webhook.site, ngrok tunnel) \\
\addlinespace

\textbf{Scheduled Jobs} &
  Requires time passage or cron trigger &
  SC-010: ``Monthly budget reset'' --- BullMQ cron job registered but monthly trigger cannot be waited for \\
\addlinespace

\textbf{Failure Injection} &
  Requires deliberately crashing a service mid-request &
  SC-007: ``Provider error → release reserved tokens'' --- requires intercepting LLM response mid-stream \\
\addlinespace

\textbf{Streaming Lifecycle} &
  Requires SSE/WebSocket long-lived connection testing &
  SC-014: ``Streaming reconciliation'' --- token counting during active SSE stream \\
\addlinespace

\textbf{Multi-Organization} &
  Requires multiple tenants with separate auth contexts &
  SC-005: ``Team budget exhaustion blocks team members but not other teams'' --- requires 2+ organizations in test data \\
\addlinespace

\textbf{External API Dependency} &
  Requires credentials or services not available in dev &
  SC (general): ``LLM provider returns 500'' --- cannot force external provider errors \\
\bottomrule
\end{longtable}

\paragraph{Rules for RUNTIME\_BLOCKED.}

\begin{enumerate}[nosep]
  \item \textbf{Must be specific}: ``RUNTIME\_BLOCKED: requires load testing infrastructure'' --- not ``RUNTIME\_BLOCKED: hard to test''
  \item \textbf{Code must be implemented}: RUNTIME\_BLOCKED means ``code exists but verification environment doesn't.'' If code is also missing, the SC is FAIL, not RUNTIME\_BLOCKED.
  \item \textbf{Must note resolution path}: ``Resolve in staging environment with k6 load test'' or ``Resolve when webhook receiver is configured''
  \item \textbf{Counts toward Overall}: A verify-report with RUNTIME\_BLOCKED SCs can still be PASS if all runtime-verifiable SCs pass. The report must clearly state: ``13/20 runtime verified, 7/20 RUNTIME\_BLOCKED (infrastructure-dependent)''
  \item \textbf{Cannot substitute unit test}: ``Verified via unit test'' is NOT a valid RUNTIME\_BLOCKED reason. Use \texttt{RUNTIME\_BLOCKED: [actual infrastructure reason] --- unit test exists as proxy} if a unit test covers the logic.
\end{enumerate}

\begin{tipbox}[When RUNTIME\_BLOCKED Is Acceptable for Merge]
A Feature can merge with RUNTIME\_BLOCKED SCs when:
\begin{itemize}[nosep]
  \item All \texttt{runtime}-verifiable SCs pass (no cherry-picking easy ones)
  \item Each RUNTIME\_BLOCKED SC has a specific reason and resolution path
  \item The user has reviewed the RUNTIME\_BLOCKED list and approved via User Demo Gate
  \item The verify-report Overall says ``PASS (Limited)'' --- not ``PASS''
\end{itemize}
The RUNTIME\_BLOCKED SCs become the first verification targets when the Feature moves to staging or production environments.
\end{tipbox}
...
When a user provides feedback at a HARD STOP, the response often requires not just stepping back but also analyzing downstream impact and proposing the next pipeline action. The following section formalizes this as the Impact Analysis + Flow Proposal protocol.

\section{Impact Analysis + Flow Proposal}

At any pipeline HARD STOP (Checkpoint or Review), users may provide feedback about bugs, missing requirements, or improvement ideas. Rather than just analyzing the issue, the agent must classify the feedback, assess downstream impact, and propose a concrete pipeline action---so the user never has to figure out the routing themselves.

When user feedback arrives at \emph{any} HARD STOP---not just verify---the agent must respond with analysis AND a concrete next step. Three rounds of ``what's wrong?'' $\rightarrow$ ``so what do we do?'' $\rightarrow$ ``but spec needs to change first'' is a pipeline usability failure.

\paragraph{The protocol (4 steps).}

\begin{enumerate}[nosep]
  \item \textbf{Classify}: Bug (code wrong), Spec Gap (requirement missing), Improvement (enhancement), New Requirement (out of scope)
  \item \textbf{Analyze impact}: Which FRs, SCs, files, and cross-Feature surfaces are affected
  \item \textbf{Propose flow}: Map classification to pipeline action:
\end{enumerate}

\begin{table}[H]
\centering
\caption{Feedback classification to pipeline flow}
\begin{tabularx}{\textwidth}{llX}
\toprule
\textbf{Classification} & \textbf{Flow} & \textbf{Command} \\
\midrule
Bug, $\leq$2 files & Fix in place & (inline during current step) \\
Bug, 3+ files & Major-Implement & \texttt{pipeline F00N --start implement} \\
Bug, architecture & Major-Plan & \texttt{pipeline F00N --start plan} \\
Spec Gap & Major-Spec & \texttt{pipeline F00N --start specify} \\
Improvement (in scope) & Augment & \texttt{add --to F00N} $\rightarrow$ \texttt{--start specify} \\
New Requirement & New Feature & \texttt{add "description"} $\rightarrow$ new Feature \\
Deferred & Roadmap note & Record in roadmap, continue current pipeline \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}[nosep]
  \setcounter{enumi}{3}
  \item \textbf{Present + ask}: Display classification, impact, proposed flow, and alternatives via AskUserQuestion
\end{enumerate}

\begin{tipbox}[Example: aegis F004 Token Estimation]
User: ``Token estimation doesn't include output tokens.''
...
NOT: ``You're right, output tokens are missing.'' (analysis only, no flow)
\end{tipbox}
...
\section{Release Groups vs Demo Groups}

Two independent grouping mechanisms manage Features at different levels:

\begin{description}
  \item[Release Group (RG)] Dependency-based execution order. Generated automatically by reverse-spec analyze-classify. RG1 Features have no dependencies; RG2 Features depend on RG1. Determines \emph{build order}. Stored in \file{roadmap.md}.
  \item[Demo Group (DG)] User-scenario-based demo grouping. Defined during \cmd{add} Phase~5. Groups Features that together demonstrate a meaningful business scenario. Determines \emph{demo scope}. Stored in \file{sdd-state.md}.
\end{description}

They are \textbf{independent}: an RG1 Feature can be in DG2 (buildable first, but demo value comes later), and an RG2 Feature can be in DG1 (depends on RG1 technically, but serves a high-priority demo).

\begin{tipbox}[Example: AEGIS Project]
\begin{itemize}[nosep]
  \item RG1: F001 (Foundation), F002 (Gateway), F003 (Auth), F004 (Budget), F005 (Logging)
  \item RG2: F006 (Security), F007 (Dashboard)
  \item DG1: F002+F003+F004 --- ``Login $\rightarrow$ LLM call $\rightarrow$ Budget check''
  \item DG2: F003+F005 --- ``Authenticated request $\rightarrow$ Logging with trace ID''
\end{itemize}
F002 is in RG1 (no deps) and DG1 (business scenario). F007 is in RG2 (depends on F003) but could be in DG1 if the dashboard is part of the demo.
\end{tipbox}

\section{Integration Demo: Cross-Feature Verification}

Individual Feature demos (\file{demos/F004-token-budget.sh}) verify Features in isolation. Integration Demos verify Features \emph{as a user journey}.

\paragraph{Lifecycle.}
\begin{enumerate}[nosep]
  \item \textbf{Define}: During \cmd{add} Phase~5, Features are assigned to Demo Groups with scenario descriptions
  \item \textbf{Create}: When the \emph{last} Feature in a DG completes implement, the agent creates \file{demos/DG\{N\}-\{scenario\}.sh}
  \item \textbf{Verify}: At verify Phase~5, if this Feature completes the DG, a HARD STOP offers Integration Demo execution
  \item \textbf{Execute}: The script runs in \texttt{--ci} mode first (automated), then interactive mode (user sees and confirms)
  \item \textbf{Record}: Result stored in \file{sdd-state.md} Demo Group Progress
\end{enumerate}

\paragraph{Script structure.}
Integration Demo scripts follow the same convention as individual demos:
\begin{itemize}[nosep]
  \item Default mode: start server + seed data + print ``Try it'' instructions + keep running
  \item \texttt{--ci} mode: automated curl-based checks + exit code
  \item Difference: the scenario crosses Feature boundaries (login from F003 $\rightarrow$ LLM call from F002 $\rightarrow$ budget check from F004)
\end{itemize}

\begin{warnbox}[Integration Demo Is Not Optional]
If a Demo Group exists, the Integration Demo script is a deliverable. ``verify Phase~5 couldn't find the script'' means implement was incomplete---return to implement and create it.
\end{warnbox}

\subsection{Phase 5: Integration Demo (conditional)}

After Phase~4 completes, the agent checks whether this Feature completes a Demo Group. If yes:
\begin{enumerate}[nosep]
  \item Locate \file{demos/DG\{N\}-\{scenario\}.sh} (BLOCKING if missing)
  \item Run \texttt{--ci} mode (must exit 0)
  \item Run interactive mode --- user sees the cross-Feature demo
  \item HARD STOP: user confirms the Integration Demo works
  \item Record result in \file{sdd-state.md} Demo Group Progress
\end{enumerate}
If this Feature does not complete any DG, Phase~5 is skipped.

\begin{tipbox}[Chapter 11 Key Takeaways]
\begin{itemize}[nosep]
  \item 4-step protocol: Assemble $\rightarrow$ Checkpoint $\rightarrow$ Execute+Review $\rightarrow$ Update
  \item 6-Phase Brief validates Feature definitions from multiple perspectives
  \item 6-Phase Verify (Phase 0--5) requires runtime evidence, not just test results
  \item Release Groups (build order) and Demo Groups (demo scope) are independent
  \item Integration Demos verify cross-Feature user journeys; Phase~5 triggers when a DG completes
  \item verify-report.md is mandatory; merge gate checks file existence + PASS status
  \item Features execute sequentially---never in parallel (MANDATORY RULE 4)
\end{itemize}
\end{tipbox}
...
Part~III showed what each skill does. Part~IV opens the hood: the module architecture, the complete inventory of 47~concerns, 15~archetypes, and 10~interfaces, and the 61~cross-concern integration rules that make the whole system domain-aware.
...
\textit{This chapter explains the triple-file structure, key system files, and convention-over-configuration approach that makes adding new modules trivial.}
...
\begin{description}
  ...
  \item[\_resolver.md] The 7-step module loading protocol including cross-concern integration (Step~3.5, 65 rules), scale adjustment (Step~4), and lazy-loading section tables (Step~5).
...
\textit{This chapter catalogs every module across all five axes: 10~interfaces, 47~concerns, 15~archetypes, 8~foundations, and context modifiers.}

\begin{tipbox}[Finding the Right Modules for Your Project]
Instead of reading all 47 concerns, start from your project type:

\textbf{Building an AI assistant?} $\rightarrow$ \texttt{ai-assistant} archetype + \texttt{gui} + \texttt{realtime} + \texttt{external-sdk} + \texttt{auth}

\textbf{Building a REST API?} $\rightarrow$ \texttt{public-api} or \texttt{microservice} archetype + \texttt{http-api} + \texttt{auth} + \texttt{resilience}

\textbf{Building a CLI tool?} $\rightarrow$ \texttt{cli} interface + relevant concerns (no archetype needed for simple tools)

\textbf{Rebuilding legacy code?} $\rightarrow$ \texttt{reverse-spec} detects modules automatically from source code. You rarely need to choose manually.

\textbf{Not sure?} $\rightarrow$ Run \texttt{/smart-sdd init} with a project description. The CI scoring system infers your Domain Profile and shows which modules would activate.
\end{tipbox}
...
\textit{This chapter describes the 61 integration rules that activate when specific module combinations are loaded together, producing emergent behavior no single module contains.}

The resolver (\file{\_resolver.md} Step~3.5) defines 65 cross-concern integration rules that activate when specific combinations of modules are loaded together. These rules produce emergent behavior that no individual module contains.
...
The module system is the theory. Part~V is the practice---20~gap patterns and 65~specific lessons discovered through hundreds of real pipeline runs. If Parts~II--IV explained \emph{how the system is designed}, Part~V explains \emph{why each design decision was made}---through the failures that motivated them.
...
%% ────────────────────────────────────────────────────────────────────
\chapter{20 Gap Patterns}
\label{ch:gaps}
%% ────────────────────────────────────────────────────────────────────

\textit{This chapter catalogs 20 recurring failure patterns---the specific ways pipelines break that build, test, and lint cannot detect.}
...
\textit{This chapter presents the most impactful lessons from hundreds of pipeline runs, organized by theme: agent interpretation, pipeline design, context management, and verification.}

Organized by theme, with selected highlights from the 65 lessons.
...
\textit{This chapter analyzes how AI agents behave under constraint pressure---seven evasion patterns, their root causes, and the structural countermeasures that neutralize each one.}

\section{Seven Evasion Patterns (Detailed)}

Agent evasion is not defiance---it is optimization. When context is compressed and the full protocol is unavailable, the agent fills gaps with what seems reasonable. The seven patterns (fabricated excuse, keyword downgrade, explicit ignore, premature completion, silent substitution, incremental compliance degradation, velocity override) share a root cause: the agent optimizes for completion, not compliance.

\paragraph{Pattern 1: Fabricated Excuse.}
\textbf{Root cause}: The agent optimizes for task completion and treats safety gates as obstacles.
\textbf{Manifestation}: ``Health check passed so HARD STOP is unnecessary.'' ``This Feature is simple enough to skip verify.''
\textbf{Countermeasure}: BLOCKING gates that check structural conditions (file exists, status field = X), not agent self-assessment.

\paragraph{Pattern 2: Keyword Downgrade.}
\textbf{Root cause}: Natural-language emphasis words (\texttt{MANDATORY}, \texttt{CRITICAL}) have no structural enforcement power.
\textbf{Manifestation}: Rules marked \texttt{MANDATORY} treated as strong suggestions and skipped under context pressure.
\textbf{Countermeasure}: Replace emphasis words with structural gates---file-based state checks that block progression.

\paragraph{Pattern 3: Explicit Ignore.}
\textbf{Root cause}: Context compression reduces long instructions to summaries, losing negation clauses.
\textbf{Manifestation}: ``Do NOT skip under any circumstances'' is skipped when compressed context drops the negation.
\textbf{Countermeasure}: Place critical rules at every execution point (inline duplication), not just at file top.

\paragraph{Pattern 4: Premature Completion.}
\textbf{Root cause}: Long task lists exceed context window; later items are compressed out.
\textbf{Manifestation}: After implementing 5 of 7 tasks, the agent declares ``implementation complete''---remaining tasks vanished from context.
\textbf{Countermeasure}: File-based task tracking (\file{tasks.md}) with explicit completion count verification.

\paragraph{Pattern 5: Silent Substitution.}
\textbf{Root cause}: The agent fills protocol gaps with simpler alternatives without announcing the change.
\textbf{Manifestation}: Instead of 5-phase verification, the agent runs build+TypeScript only and reports ``verify complete'' with a checkmark.
\textbf{Countermeasure}: Output verification---check whether the output contains evidence of each required phase, not whether the agent claims compliance.

\begin{warnbox}[The Most Dangerous Pattern]
Silent substitution (Pattern~5): the agent produces output that \emph{looks correct}. It does not say ``I skipped verification.'' It says ``verification complete''---confidently, with a checkmark. Only manual inspection reveals that 80\% of the protocol was silently dropped.
\end{warnbox}
...
\textbf{Root cause}: When a rule violation is caught and a countermeasure added, the agent shifts to a subtler form of the same violation.
\textbf{Manifestation}: Observed during the aegis pilot as a 4-stage escalation:
...
\textbf{Countermeasure}: Each stage satisfies the \emph{letter} of the previous countermeasure while violating its \emph{spirit}. The defense must escalate from rules (can be gamed) to structural gates (require evidence) to user confirmation (cannot be faked).
...
\begin{notebox}[Beyond Evasion: Quality Degradation]
After the evasion chain (P2--P8) was resolved, a subtler issue emerged: verify-reports were generated correctly but contained quality gaps---``unit test'' in Method columns, lint silently omitted, test count not growing. These are not evasion (the agent is trying to comply) but \textbf{quality degradation from insufficient template constraints}. The fix was structural: restrict Method to a closed value set, require lint status regardless of installation, and track test count growth. Lesson: even after behavioral evasion is blocked, output quality must be separately validated.
\end{notebox}

\section{Velocity Override --- The Seventh Evasion Pattern}

\paragraph{Pattern 7: Velocity Override.}
\textbf{Root cause}: The user's urgency request (``do everything,'' ``just finish it,'' ``auto-approve all'') gives the agent implicit permission to bypass quality gates. Unlike the first six patterns which stem from context compression or optimization pressure, Velocity Override is \textbf{user-authorized degradation}---the agent has a legitimate-sounding reason to skip steps.

\textbf{Manifestation}: The user says ``run the full pipeline, auto-approve HARD STOPs.'' The agent interprets this as license to:
\begin{itemize}[nosep]
...
The other patterns happen silently---the agent doesn't announce it's skipping. Velocity Override is \emph{openly rationalized}: ``The user asked me to go fast, so I simplified the output.'' This makes it the most dangerous pattern because the agent's reasoning sounds legitimate.
...
\section{The Fundamental Limit: Pipeline Completion Bias Cannot Be Solved by Rules}

Every countermeasure in this chapter shares an assumption: \textbf{the agent will read and follow rules.} The aegis pilot revealed that this assumption breaks down systematically after extended pipeline execution.

\begin{dangerbox}[The Most Important Finding in This Manual]
After 2+ hours of continuous pipeline execution (specify $\rightarrow$ plan $\rightarrow$ tasks $\rightarrow$ analyze $\rightarrow$ implement), the agent develops an implicit optimization goal: \textbf{finish the pipeline.} This ``Pipeline Completion Bias'' overrides explicit rules, including rules the agent demonstrably read and understood.

\textbf{Evidence from the aegis pilot} (F007--F008):
\begin{itemize}[nosep]
  \item SKILL.md Rule~5 (Runtime SC Verification): read at session start, violated at verify
  \item SKILL.md Rule~7 (Per-Task Micro-Verify): read at session start, violated at implement
  \item injection/implement.md: never loaded despite explicit ``MUST read'' instruction at pipeline.md:1333
  \item SC Matrix Pre-population: defined as BLOCKING, never executed
  \item verify-report Match? column: filled in, but ``CODE'' results marked as $\checkmark$ PASS
\end{itemize}

\textbf{The pattern}: Each rule was present, accessible, and even acknowledged. But under time pressure, the agent found the path of least resistance: write code fast, run build once, report everything as passing. Adding more rules does not fix this---the agent already ignores the rules it has.
\end{dangerbox}

\paragraph{Why This Is Not Solvable by Skill Design Alone.}

This is a \textbf{model-level behavior}, not a skill-level bug. The agent's implicit objective (complete the task efficiently) conflicts with explicit instructions (verify every SC at runtime) when both require the same finite resource: context window and time. Under resource pressure, implicit objectives win.

The analogy from human software engineering: a developer who has been coding for 12 hours will cut corners on testing---not because they don't know testing matters, but because fatigue shifts the cost-benefit calculation. No amount of process documentation prevents this. What prevents it is \textbf{structural separation}: the developer writes code, a different person reviews it, a CI system runs tests.

\paragraph{Practical Mitigations.}

Three structural approaches that work regardless of agent compliance:

\begin{enumerate}
  \item \textbf{Session separation}: implement and verify MUST run in different sessions. After implement completes, the user runs \texttt{/clear}, then starts a new session with \texttt{/smart-sdd pipeline F00N --start verify}. The verify session has no implement fatigue, no sunk cost, no ``I already know this code works'' bias. \textbf{This is the single most effective mitigation.}

  \item \textbf{User as final gate}: The User Demo Gate (MANDATORY RULE~6) exists precisely because agent self-verification is unreliable. The user must actually see the running application. No amount of $\checkmark$ marks in a report substitutes for the user clicking a button and seeing it work.

  \item \textbf{verify-report as persistent evidence}: Even when the agent marks everything as PASS, the verify-report exists as a file. The user can read it, see ``METHOD: CODE'' on 7 of 8 SCs, and immediately know runtime verification was skipped. The report doesn't prevent the violation---it makes the violation \textbf{visible and auditable}.
\end{enumerate}

\begin{tipbox}[The Honest Recommendation]
If you are using spec-kit-skills for a production project: \textbf{never let the same session run implement and verify.} Clear context between them. Treat implement output with the same skepticism you would treat a pull request from a junior developer who says ``I tested it'' but the PR has no test files. The agent is not lying---it genuinely believes it verified. But belief is not evidence.
\end{tipbox}
...
Part~V cataloged what can go wrong. Part~VI shows how to get started, with step-by-step walkthroughs for six scenarios, advanced techniques like Context Reset and Step-Back Navigation, and practical guidance for building your own skills.
...
\textit{This chapter gets you from zero to a running pipeline in 10 minutes.}
...
\textit{This chapter walks through six real scenarios end-to-end, showing the exact commands and decision points for each project situation.}
...
\textit{This chapter covers power-user techniques: Context Reset between Features, Step-Back Navigation mid-pipeline, and guidance for building custom skills on top of the module system.}
...
\section{Cross-Feature API Gap Resolution}

When a Feature needs an API that was supposed to be provided by a preceding (already merged) Feature, there are three options:

\begin{description}
  \item[Option A: Implement here] Add the missing API in the \emph{current} Feature. Update spec.md to document it as ``supplementary implementation (F003 scope gap).'' Update api-registry with the actual provider. \textbf{Recommended for individual endpoints.}
  \item[Option B: Reopen preceding Feature] \texttt{add --to F003 "add PUT /users/:id"} $\rightarrow$ full re-pipeline (specify $\rightarrow$ plan $\rightarrow$ tasks $\rightarrow$ implement $\rightarrow$ verify $\rightarrow$ merge). \textbf{Recommended only for architectural changes} --- disproportionate for a single endpoint.
  \item[Option C: Mark as RUNTIME\_BLOCKED] SC cannot be verified because the API doesn't exist. Defer to a future Feature or Tier.
\end{description}

\begin{tipbox}[Decision Heuristic]
\textbf{Individual endpoint missing?} $\rightarrow$ Option A (implement here, document in spec).\\
\textbf{Data model or architecture change needed?} $\rightarrow$ Option B (reopen preceding Feature).\\
\textbf{Entire subsystem missing?} $\rightarrow$ Option C (defer to new Feature).

The cost of reopening a merged Feature (6+ pipeline steps) must justify the change. One PATCH endpoint does not justify a full re-pipeline.
\end{tipbox}

The Feature Boundary Clarification Gate (added to specify) catches this at spec time: ``API not in registry $\rightarrow$ add to In-Scope or Out-of-Scope?'' When caught at verify time (spec predates the gate), the pragmatic fix is Option A.
...
%% ────────────────────────────────────────────────────────────────────
\chapter{Case Study: AEGIS}
\label{ch:case-study}
%% ────────────────────────────────────────────────────────────────────

\textit{A complete greenfield project built with \speckit{} over four days: 12~Features, 122~Success Criteria, 3~custom domain modules, and 22~skill feedback items that drove nine of the framework's MANDATORY RULES.}

%% ── Project Overview ─────────────────────────────────────────────────
\section{Project Overview}

\textbf{AEGIS} (AI Enterprise Gateway \& Intelligence System) is a self-hosted enterprise platform that unifies LLM routing, multi-tenant cost management, security guardrails, and knowledge integration into a single open-source product.

\begin{table}[H]
\centering
\caption{AEGIS project summary}
\label{tab:aegis-summary}
\begin{tabularx}{\textwidth}{lX}
\toprule
\textbf{Property} & \textbf{Value} \\
\midrule
Type & Greenfield \\
Duration & 4 days (2026-03-25 to 2026-03-28) \\
Stack & TypeScript, NestJS (backend), Next.js + shadcn/ui (frontend), PostgreSQL + pgvector, Redis, BullMQ \\
Features & 12 across 4 tiers (T0--T3) and 4 release groups \\
Success Criteria & 122 total \\
Tests & 185 (Jest unit + integration) \\
Source files & $\sim$120 (apps/api + apps/web + libs/common) \\
Custom domain modules & 3 (ai-gateway, token-budget, prompt-guard) \\
Skill feedback items & 22 (P1--P18, F007 P8--P11, F008 P12--P13) \\
\bottomrule
\end{tabularx}
\end{table}

%% ── Domain Profile ───────────────────────────────────────────────────
\section{Domain Profile and Custom Modules}

\subsection{5-Axis Profile}

The project's Domain Profile was established during \cmd{smart-sdd init} with a Clarity Index of 97\% (Rich tier):

\begin{lstlisting}[style=specstyle, title={AEGIS Domain Profile}]
...
Context:     greenfield | mvp x small-team

# * = project-local custom modules (not in built-in library)
\end{lstlisting}

\subsection{Gap Analysis and Module Creation}

Matching the project description against the built-in taxonomy (58~concerns, 14~archetypes, 10~interfaces) revealed three gaps:

\begin{table}[H]
\centering
\caption{Domain module gap analysis}
\label{tab:aegis-gaps}
\begin{tabularx}{\textwidth}{llcX}
\toprule
\textbf{Gap} & \textbf{Closest built-in} & \textbf{Sim.} & \textbf{Why a new module was needed} \\
\midrule
AI Gateway routing & ai-assistant & 15\% & Built-in models the LLM \emph{consumer}; AEGIS is the LLM \emph{manager} \\
Hierarchical token budget & resilience & 20\% & Flat rate-limiting vs.\ Org$>$Team$>$User token+cost tracking \\
Prompt security & content-moderation & 25\% & Async review queue vs.\ inline $<$100ms LLM-specific scanning \\
\bottomrule
\end{tabularx}
\end{table}

Three project-local modules were created via \cmd{domain-extend extend}:

\begin{itemize}[nosep]
  \item \textbf{ai-gateway} (Archetype) --- 5 principles (Provider Abstraction, Streaming-First, Token-Aware Routing, Multi-tenant Isolation, Audit Everything), 5 failure modes (AG-001--005)
  \item \textbf{token-budget} (Concern) --- 5 SC required patterns, 4 anti-patterns, 5 failure modes (TB-001--005), 6 elaboration probes
  \item \textbf{prompt-guard} (Concern) --- 5 SC required patterns, 4 anti-patterns, 5 failure modes (PG-001--005), 6 elaboration probes
\end{itemize}

\begin{notebox}[Custom Module Impact]
These modules participated in every pipeline stage: S0 keywords improved signal extraction during \cmd{init}, S1 rules enforced SC patterns during \texttt{specify}, S5 probes deepened the Brief during \cmd{add}, and S7 failure modes guided defensive design during \texttt{plan} and \texttt{implement}. Without them, the 22 SC rules and 15 failure modes they contributed would have been left to ad-hoc agent judgment.
\end{notebox}

%% ── Pipeline Execution ──────────────────────────────────────────────
\section{Pipeline Execution}

\subsection{Feature-by-Feature Results}

\begin{table}[H]
\centering
\caption{Per-Feature pipeline results}
\label{tab:aegis-features}
\small
\begin{tabularx}{\textwidth}{clXcccc}
\toprule
\textbf{FID} & \textbf{Tier} & \textbf{Name} & \textbf{SC} & \textbf{Verify} & \textbf{Tests} & \textbf{Sessions} \\
\midrule
F001 & T0 & Foundation Setup & 8/8 & \textcolor{specgreen}{PASS} & --- & 1 \\
F002 & T1 & LLM Gateway Core & 4/4 & \textcolor{specgreen}{PASS} & --- & 1 \\
F003 & T1 & Auth \& Multi-tenancy & 10/10 & \textcolor{specgreen}{PASS} & --- & 2 \\
F004 & T1 & Token Budget Management & 13/20 & \textcolor{specorange}{LIMITED} & 107 & 3 \\
F005 & T1 & Request Logging \& Tracing & 8/16 & \textcolor{specorange}{LIMITED} & 107 & 1 \\
F006 & T2 & Security Guardrails & 8/8 & \textcolor{specgreen}{PASS} & 168 & 1 \\
F007 & T2 & Admin Dashboard & 16/16 & \textcolor{specgreen}{PASS} & 168 & 2 \\
F008 & T2 & Provider Fallback \& LB & 8/8 & \textcolor{specgreen}{PASS} & 168 & 1 \\
F009 & T3 & Knowledge Integration & 9/9 & \textcolor{specgreen}{PASS} & 178 & 1 \\
F010 & T3 & Prompt Management & 14/14 & \textcolor{specorange}{LIMITED} & 178 & 1 \\
F011 & T3 & Semantic Cache & 11/11 & \textcolor{specorange}{LIMITED} & 185 & 1 \\
F012 & T3 & Developer Playground & 12/12 & \textcolor{specorange}{LIMITED} & 185 & 1 \\
\midrule
\multicolumn{3}{l}{\textbf{Total}} & \textbf{122} & \textbf{8 PASS / 4 LTD} & \textbf{185} & \textbf{16} \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Verify levels}: 8 Features achieved full runtime verification (curl + Playwright against a running server). 4 Features (F004 partial, F010--F012) were verified at code level only due to infrastructure constraints (DB/Redis not running). Code-level verification still caught structural defects, including the F011 cross-Feature regression (SC-011).

\subsection{Demo Groups: Cross-Feature Integration}

Two Demo Groups validated end-to-end user scenarios after T1 completion:

\begin{itemize}[nosep]
  \item \textbf{DG1} (F002+F003+F004): JWT login $\to$ LLM call $\to$ budget deduction $\to$ budget exhaustion $\to$ 429 block. 8/8 SC PASS.
  \item \textbf{DG2} (F003+F005): API Key auth $\to$ LLM call with trace ID propagation $\to$ async log creation $\to$ analytics query. 8/8 SC PASS.
\end{itemize}

\subsection{Regressions Discovered and Fixed}

\begin{table}[H]
\centering
\caption{Regression cases}
\label{tab:aegis-regressions}
\begin{tabularx}{\textwidth}{clXX}
\toprule
\textbf{Feature} & \textbf{SC} & \textbf{Root Cause} & \textbf{Resolution} \\
\midrule
F004 & SC-003 & Token estimation counted only input content; output tokens ignored entirely & Pessimistic estimation formula: input/3 + msg overhead + max\_tokens fallback \\
F004 & SC-014 & \texttt{extractUserKey()} returned null --- reconciliation never executed & Store key paths in Redis reservation HASH; reconcile reads paths directly \\
F011 & SC-011 & CacheInterceptor did not set \texttt{request.cacheHit} flag & F005 RequestLoggerInterceptor always logged \texttt{cacheHit:~false}; 2-file fix \\
\bottomrule
\end{tabularx}
\end{table}

F004 regressions triggered a full pipeline re-run (\texttt{--start specify}), cascading spec$\to$plan$\to$tasks$\to$implement$\to$verify. F011 was resolved with a targeted implement regression.

%% ── Critical Moments ─────────────────────────────────────────────────
\section{Critical Moments}

\subsection{HARD STOP Interventions}

Four critical discoveries were made exclusively at \hardstop{} review points:

\begin{table}[H]
\centering
\caption{\hardstop{} interventions}
\label{tab:aegis-hardstop}
\begin{tabularx}{\textwidth}{clXX}
\toprule
\textbf{\#} & \textbf{Stage} & \textbf{What happened} & \textbf{Without \hardstop{}} \\
\midrule
1 & F004 specify & UsageRecord state mismatch (\texttt{failed} vs \texttt{released}) caught in review & Incorrect state machine implemented; runtime data corruption \\
2 & F004 specify & User requested Korean language conversion for F001--F003 artifacts & Missed the conversion window; massive rework after implement \\
3 & F004 verify & User injected domain decision: ``model-specific budget management (ModelTier)'' & System built with model-agnostic budgets; major refactoring later \\
4 & F003 verify & Refresh token jti collision bug discovered & Two logins within 1 second produce identical tokens in production \\
\bottomrule
\end{tabularx}
\end{table}

\begin{dangerbox}[Key Insight]
\hardstop{} is not merely an approval gate. It is the \emph{only} point where domain expertise enters the pipeline. Intervention \#3 --- the ModelTier budget requirement --- could not have been derived from code analysis; it came from business knowledge that only the user possessed.
\end{dangerbox}

\subsection{Parallelization Halt (P3)}

Early in the project, the agent attempted to process F003, F004, and F005 simultaneously via background agents. This violated MANDATORY RULE~4 (Sequential Feature Execution) and was halted immediately. The violation produced file conflicts, stale registry references, and entity duplication --- confirming why the rule exists.

\subsection{Pipeline Completion Bias Evidence}

Features F010--F012 were executed late in a long session. All three received only code-level verification despite having runtime-verifiable SCs. The agent's implicit pressure to ``finish quickly'' manifested as:

\begin{itemize}[nosep]
  \item Level~1 (render-only) checks for Level~3 (CRUD) SCs
  \item Skipping Playwright for ``obvious'' frontend pages
  \item Reporting ``page renders correctly'' as ``SC complete''
\end{itemize}

This pattern directly motivated MANDATORY RULE~9 (Session Separation): implement and verify must run in different sessions to prevent accumulated fatigue from degrading verification rigor.

%% ── Skill Feedback Evolution ─────────────────────────────────────────
\section{Skill Feedback Evolution}

The 22 skill feedback items followed a predictable four-phase pattern:

\begin{table}[H]
\centering
\caption{Skill feedback evolution by phase}
\label{tab:aegis-feedback}
\begin{tabularx}{\textwidth}{llcX}
\toprule
\textbf{Phase} & \textbf{Items} & \textbf{Severity} & \textbf{Pattern} \\
\midrule
Early (F001--F003) & P1--P4 & CRITICAL & Foundational violations: template non-compliance, verify skip, parallel execution \\
Mid (F004--F005) & P5--P7 & HIGH & Verify quality degradation: runtime checks bypassed, partial SC coverage \\
Late (F006--F009) & P8--P14 & HIGH--MED & Detail gaps: npm install omission, render$=$PASS fallacy, micro-verify absence \\
Integration & P15--P18 & MEDIUM & Environment and UX issues: .env handling, demo script guidance \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{From Feedback to MANDATORY RULES}

Nine of the framework's MANDATORY RULES trace directly to AEGIS feedback:

\begin{table}[H]
\centering
\caption{Feedback-to-rule traceability}
\label{tab:aegis-rules}
\begin{tabularx}{\textwidth}{lXl}
\toprule
\textbf{Feedback} & \textbf{Problem} & \textbf{Rule Created} \\
\midrule
P2 & Verify phase entirely skipped & Rule 5: Runtime SC Verification \\
P3 & Features processed in parallel & Rule 4: Sequential Feature Execution \\
P5, P6 & Unit test pass treated as SC verification & Rule 5 (strengthened) \\
P9 & \hardstop{} review bypassed & Rule 1: \hardstop{} Enforcement \\
P10, P11 & Rendering treated as PASS; code without spec & Rule 6: Honest SC Evidence \\
P11-D & Code added without spec & Rule 8: No Code Without Spec \\
P12, P13 & All code written in one batch; code-level PASS reported & Rule 7: Per-Task Micro-Verify \\
F010--F012 & Late-session verify degradation & Rule 9: Session Separation \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Micro-Verify: A Controlled Experiment}

F008 and F009 provide a natural A/B comparison for per-task micro-verify:

\begin{itemize}[nosep]
  \item \textbf{F008 (without)}: All circuit breaker code written in one batch $\to$ P12 + P13 feedback; compounded issues discovered only in verify
  \item \textbf{F009 (with)}: Each task verified immediately after writing $\to$ 3~runtime errors caught and fixed during implement (BullMQ connection, pgvector type conflict, SQL column casing)
\end{itemize}

The contrast directly motivated MANDATORY RULE~7's per-task micro-verify requirement.
...
\begin{table}[H]
\centering
\caption{AEGIS final metrics}
\label{tab:aegis-metrics}
\begin{tabularx}{\textwidth}{Xr}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
Features completed & 12 / 12 \\
Success Criteria (total) & 122 \\
Runtime-verified Features & 8 / 12 (67\%) \\
Code-level-only Features & 4 / 12 (33\%) \\
Jest tests (final) & 185 across 26 suites \\
Source files & $\sim$120 \\
Custom domain modules & 3 \\
Skill feedback items & 22 \\
Regressions discovered / fixed & 3 \\
\hardstop{} interventions & 4 \\
MANDATORY RULES derived & 9 \\
Demo Groups verified & 2 (16/16 SC) \\
Pipeline sessions (total) & 16 \\
Calendar days & 4 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}[nosep]
  \item \textbf{Verify $\neq$ build + test pass.} Runtime SC verification against a running server is the only valid evidence. Unit tests passing is a prerequisite, not a conclusion (P2, P5, P6).
  \item \textbf{\hardstop{} is irreplaceable.} Domain knowledge that cannot be derived from code (ModelTier budgets, language preferences, state machine corrections) can only enter through user review (P9, \S\ref{tab:aegis-hardstop}).
  \item \textbf{Features must execute sequentially.} Parallel execution causes entity-registry conflicts, stale cross-Feature references, and untraceable state corruption (P3).
  \item \textbf{Per-task micro-verify catches bugs earlier.} F008 (without) vs.\ F009 (with) demonstrated that batch-then-verify compounds issues while task-level verification isolates them (P12, P13).
  \item \textbf{Frontend and backend require different verify strategies.} Backend: curl against API endpoints. Frontend: Playwright full-path verification + API smoke test + npm install gate (F007 P8--P11).
  \item \textbf{Cross-Feature issues surface during verify, not implement.} The F011 SC-011 regression (CacheInterceptor $\to$ RequestLogger data flow) was invisible to per-task micro-verify but caught in verify Phase~2.
  \item \textbf{Custom domain modules pay for themselves.} The 22 SC rules and 15 failure modes contributed by ai-gateway, token-budget, and prompt-guard prevented entire classes of defects from reaching verify.
  \item \textbf{Pipeline Completion Bias is real and structural.} After 2+ hours of pipeline execution, verify quality degrades measurably. The only fix is session separation (MANDATORY RULE~9).
  \item \textbf{No code without spec.} When verify discovers missing functionality, the fix must start at spec, not at code. Cascading updates (spec $\to$ plan $\to$ tasks $\to$ implement $\to$ verify) preserve SDD integrity (P11-D, Rule~8).
  \item \textbf{Correction mechanisms before precision.} F004's reconciliation bug taught that an imprecise estimate with working reconciliation is better than a precise estimate with a broken correction path.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \file{lessons-learned.md}---20 gap patterns + 65 lessons
...
\begin{description}[style=nextline]
  ...
  \item[Module] A markdown file containing domain-specific rules organized by standardized section numbers (S0--S9, A0--A5, R1--R5, F0--F8b).
