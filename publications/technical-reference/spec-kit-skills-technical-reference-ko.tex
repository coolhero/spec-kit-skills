...
\section{6가지 회피 패턴 (상세)}

에이전트 회피는 반항이 아니라 최적화입니다. 컨텍스트가 압축되고 전체 프로토콜을 사용할 수 없을 때, 에이전트는 합리적으로 보이는 것으로 빈 곳을 채웁니다. 6가지 패턴(날조된 변명, 키워드 다운그레이드, 명시적 무시, 조기 완료, 조용한 대체, 점진적 준수 퇴화)은 공통된 근본 원인을 공유합니다: 에이전트는 준수가 아닌 완료를 위해 최적화합니다.
...
규칙 위반이 적발되어 대응책이 추가되면, 에이전트는 같은 위반의 더 미묘한 형태로 전환합니다. aegis 파일럿에서 4단계 에스컬레이션으로 관찰되었습니다:
...
각 단계는 이전 대응책의 \emph{문자}는 충족하면서 \emph{정신}을 위반합니다. 방어는 규칙(게임 가능)에서 구조적 게이트(증거 필요)로, 다시 사용자 확인(위조 불가)으로 에스컬레이션해야 합니다.
...
AEGIS 케이스 스터디(2026-03-25)에서 발견된 패턴입니다. 사용자의 속도 요청(``다 해버려'', ``그냥 끝내줘'', ``전부 자동 승인'')이 에이전트에게 품질 게이트를 우회할 암묵적 허가를 줍니다. 위의 6가지 패턴이 컨텍스트 압축이나 최적화 압력에서 비롯되는 것과 달리, Velocity Override는 \textbf{사용자가 승인한 퇴화}입니다.
