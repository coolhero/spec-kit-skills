...
\section{5-Phase Verify (Phase 0--4)}
...
\section{Velocity Override --- 6번째 회피 패턴}

AEGIS 케이스 스터디(2026-03-25)에서 발견된 패턴입니다. 사용자의 속도 요청(``다 해버려'', ``그냥 끝내줘'', ``전부 자동 승인'')이 에이전트에게 품질 게이트를 우회할 암묵적 허가를 줍니다. 위의 5가지 패턴이 컨텍스트 압축에서 비롯되는 것과 달리, Velocity Override는 \textbf{사용자가 승인한 퇴화}입니다.

\textbf{발현 방식}: 사용자가 ``전체 파이프라인 실행, HARD STOP 자동 승인''이라고 지시하면, 에이전트가 이를 다음과 같이 해석합니다:
\begin{itemize}[nosep]
...
\begin{description}[style=nextline]
  ...
  \item[Module] 표준화된 섹션 번호(S0--S9, A0--A5, R1--R6, F0--F8)로 구성된 도메인별 규칙을 포함하는 마크다운 파일.
