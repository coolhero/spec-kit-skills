...
Archetype은 A-sections (A0--A5)를, reverse-spec은 R-sections (R1--R5)를 사용한다. 전체 스키마는 Chapter~\ref{ch:sections} 참조. 지금은 \textbf{번호가 Pipeline의 어느 시점에 사용되는지를 나타낸다}는 것만 기억하면 된다.
...
프로젝트에는 19가지 갭 패턴과 53건 이상의 구체적 교훈이 포함되어 있으며, 각각은 실제 사건으로 추적된다. 이론이 아니다---모든 대응책은 관찰된 실패에 의해 동기부여되었다. 카탈로그 자체가 기여다: 이러한 패턴은 도구에 관계없이 AI 에이전트 워크플로우를 구축하는 모든 사람에게 적용된다.
...
S6 & Pattern Rules & \texttt{plan}/\texttt{implement}: 패턴 제약과 관용구 \\
...
\section{R 패밀리: Reverse-Spec 분석 (R1--R5)}
...
R4 & Data Flow Rules & 이 모듈에 특화된 엔티티 및 API 추출 규칙 \\
R5 & Feature Boundary Heuristics & 코드에서 Feature 경계를 식별하는 방법 \\
\bottomrule
\end{tabularx}
\end{table}

\section{F 패밀리: Foundation 플랫폼 (F0--F8)}
...
F1 & (예약) & --- \\
F2 & Specify-time Items & 스펙 생성에 영향을 미치는 프레임워크 제약 \\
F3 & Implement-time Rules & 프레임워크 관용구, 안티패턴, 도구 체인 설정 \\
...
verify & S1 (SC 준수 검사용), S3, S7, S8, F8, F9 \\
...
\textit{이 챕터는 Feature 간 공유 메모리 역할을 하는 4개의 파일을 설명한다. 이 파일들이 없으면 매 Feature가 제로에서 시작한다.}
...
S3 (Verify Steps) & 빌드 + 스모크 테스트 & 전체 4-Phase 검증 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{description}
  \item[Phase 0: 전략] 사용자가 범위(core/full), 스택(same/new), 네이밍 질문에 답합니다.
  \item[Phase 1: 코드 패턴 분석] 파일 구조 스캔, 의존성 그래프, 아키텍처 패턴 식별. 구조적 맵을 생성합니다.
  \item[Phase 1.5: 런타임 탐색] 소스 앱을 실행하고 실제 UI 흐름을 캡처합니다. 리빌드 모드에서 필수입니다. 스크린샷과 인터랙션 녹화를 생성합니다.
  \item[Phase 2: Source Behavior Inventory (SBI)] 모든 사용자 대면 동작을 B\#\#\# 항목으로 카탈로그화하고 소스 코드 위치에 매핑합니다.
  \item[Phase 3: Entity 및 API 추출] smart-sdd와 동일한 형식으로 \file{entity-registry.md}와 \file{api-registry.md}를 생성합니다.
  \item[Phase 4: 로드맵 구성] 동작을 의존성 순서로 Feature에 그룹화합니다. \file{roadmap.md}를 생성합니다.
  \item[Phase 5: Constitution 시드] 아키텍처 원칙을 추출합니다. \file{constitution-seed.md}를 생성합니다.
\end{description}
...
가장 복잡한 Pipeline 단계로, 4개 Phase로 나뉩니다 (Section~\ref{sec:4-phase-verify}에서 설명).
...
\section{5가지 회피 패턴 (상세)}

에이전트 회피는 반항이 아니라 최적화입니다. 컨텍스트가 압축되고 전체 프로토콜을 사용할 수 없을 때, 에이전트는 합리적으로 보이는 것으로 빈 곳을 채웁니다. 5가지 패턴(날조된 변명, 키워드 다운그레이드, 명시적 무시, 조기 완료, 조용한 대체)은 공통된 근본 원인을 공유합니다: 에이전트는 준수가 아닌 완료를 위해 최적화합니다.

\begin{warnbox}[가장 위험한 패턴]
조용한 대체: 에이전트가 \emph{올바르게 보이는} 출력을 생성합니다. ``검증을 건너뛰었습니다''라고 말하지 않습니다. ``검증 완료''라고---자신있게, 체크마크와 함께 말합니다. 수동 검사를 해야만 프로토콜의 80\%가 조용히 빠졌다는 것을 알 수 있습니다.
\end{warnbox}
...
\begin{itemize}
  ...
  \item \file{lessons-learned.md}---19가지 gap 패턴 + 53개 이상의 교훈
...
\begin{description}[style=nextline]
  ...
  \item[Module] 표준화된 섹션 번호(S0--S9, A0--A5, R1--R5, F0--F8)로 구성된 도메인별 규칙을 포함하는 마크다운 파일.
