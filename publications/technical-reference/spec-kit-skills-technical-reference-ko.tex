...
AI 코딩 도구를 써본 개발자라면 누구나 겪어본 시나리오다. AI 에이전트에게 사용자 인증 기능을 추가해달라고 요청한다. 에이전트가 30초 만에 400줄의 코드를 내놓는다. 리뷰해본다. 괜찮아 보인다. 머지한다.

3주 후, 팀이 세션 기반으로 표준화했는데 JWT를 쓴 걸 발견한다. 토큰을 \texttt{localStorage}에 저장해 XSS 공격에 노출되어 있다. Feature~2에서 이미 정의한 \texttt{User} 모델과 충돌하는 \texttt{User} 모델을 새로 만들었다. 에러 처리는 정상 경로만 다루고 나머지는 비어 있다.

코드는 \emph{동작했다}. 테스트도 있었다. 하지만 그것은 \emph{당신의} 코드가 아니었다---단지 당신의 저장소에 존재하게 된 코드일 뿐이었다.

이것이 AI 능력과 개발자 제어 사이의 간극이다. 프롬프트는 일회성 지시에 불과하다. Feature~1에서 뭘 결정했는지 기억하지 못하고, 팀의 아키텍처 패턴을 인식하지 못하며, 프로젝트가 웹 앱이 아니라 Electron 앱이라 상태 관리 방식이 달라진다는 점도 이해하지 못한다.

정작 필요한 건 더 나은 프롬프트가 아니라 다음을 수행하는 \textbf{시스템}이다:
...
이를 \textbf{Harness Engineering}이라 부른다---AI 에이전트의 행동을 신뢰할 수 있는 결과로 유도하는 구조화된 시스템을 구축하는 분야다.

\section{능력과 제어 가능성 사이의 간극}

에이전틱 코딩 도구는 이미 임계점을 넘었다. Claude Code, Cursor, Copilot Workspace, Devin---이들은 줄 단위 자동완성에 그치지 않고 전체 Feature를 계획하고, 구현하고, 테스트하고, 반복한다.

하지만 에이전트가 똑똑해질수록 감독 없이 더 큰 피해를 줄 수 있다. 기본적인 자동완성 제안은 한 줄이라 리뷰하기 쉽다. 반면 12개 파일에 걸쳐 인증 시스템 전체를 스캐폴딩하는 에이전틱 워크플로우는 전혀 다른 리뷰가 필요하다. 출력만이 아니라 그것을 생성한 \emph{프로세스} 자체를 신뢰할 수 있어야 한다.

업계가 향하는 방향이 바로 이것이다. ``AI가 개발자를 대체한다''가 아니라 ``개발자가 AI를 위한 하네스를 구축한다.'' 가치의 중심이 \emph{코드 작성}에서 \emph{코드를 작성하는 시스템 설계}로 이동한다.

\section{실제 실패 사례}

\speckit{}을 만들게 된 실패들은 가설이 아니다. 프로젝트 개발 과정의 실제 Pipeline 실행에서 하나하나 드러났다---3주간 500건 이상의 커밋을 거치며 20가지 반복 실패 패턴과 50건 넘는 구체적 사건이 기록되었다.

\subsection{``빌드 통과'' 함정}

빌드가 통과했다. TypeScript도 깨끗했다. 테스트도 녹색이었다. 그런데 앱은 완전히 망가져 있었다---상태 셀렉터가 매 프레임 새 객체 참조를 생성해 무한 리렌더링이 발생했고, 스트리밍 중 스크롤이 깨졌으며, UI가 데드 스파이럴에 빠졌다.
...
5개 Feature가 Playwright UI 검사까지 완벽히 검증된 후, Feature~6에서는 Playwright가 통째로 생략되었다. 컨텍스트 압축이 검증 지침을 삼켜버린 것이다. 같은 에이전트, 같은 스킬, 같은 Pipeline---대화가 길어졌을 뿐인데. 에이전트는 \texttt{npm run build}만 돌리고 에러 0개를 확인한 뒤 ``verify complete''를 보고하며 넘어갔다. 검증의 4개 Phase가 조용히 건너뛰어진 것이다.
...
가드레일 없이 더 유능해진 에이전트는 \emph{리뷰하기 더 어렵고} \emph{더 인상적으로 보이는} 출력을 만들어낸다. 에이전트가 30초 만에 12개 파일의 인증 코드를 쏟아내면 각 줄을 리뷰할 수 없다. 프로세스를 신뢰해야 하는데, 프로세스에 구조적 보장---차단하는 게이트, 기억하는 레지스트리, 추적하는 상태 머신---이 갖춰져야만 신뢰할 수 있다.

\section{분야의 정의: 에이전트를 위한 소프트웨어 엔지니어링}

spec-kit-skills는 \textbf{에이전트를 위한 소프트웨어 엔지니어링}에 대한 최초의 본격적 시도 중 하나다---에이전트가 작성하는 소프트웨어가 아니라, 에이전트가 소프트웨어를 \emph{어떻게} 작성하는지를 관장하는 소프트웨어다. 이 구분이 중요하다:
...
AI 에이전트에게 ``전환''이 뭘 의미하는지 알기 전까지는 당연하게 들린다: Feature 간(Feature~3는 깨끗한 컨텍스트로 시작), Pipeline 단계 간(specify에서 plan으로 넘어가며 세부사항이 빠질 수 있음), 세션 간(사용자가 밤에 노트북을 닫음). 전환마다 정보가 유실될 위험이 있다.

\subsection{P1-a: Domain Profile을 일급 시민으로}

Domain Profile(5축: Interface, Concern, Archetype, Foundation, Context)은 한번 설정하고 잊는 값이 아니다. 모든 Pipeline 단계에 적극적으로 영향을 준다---\texttt{add}에서 어떤 프로브를 질문할지, \texttt{specify}에서 어떤 규칙을 활성화할지, \texttt{verify}에서 어떤 검증 단계를 실행할지를 결정한다.
...
이 원칙이 Context Reset Protocol을 가능하게 합니다---모든 상태가 파일에 있으므로 작업 단위 간 컨텍스트를 비워도 안전합니다. 전체 절차는 Chapter~\ref{ch:advanced} \S~Context Reset Protocol을 참조하세요.
...
\begin{warnbox}[P2 보조 원칙: 줄 위치가 중요하다 (L88)]
P2는 파일 \emph{간}(참조 vs 인라인)뿐 아니라 파일 \emph{내}에도 적용된다. 2000줄 파일의 1453줄에 있는 규칙은 준수율이 거의 0이다 --- 에이전트는 처음 800\textasciitilde1000줄만 읽고 멈춘다. aegis 파일럿에서 Per-Task Micro-Verify가 \texttt{pipeline.md:1453}과 \texttt{injection/implement.md:598}에 정의되어 있었는데, 에이전트는 두 줄 모두 도달하지 못했다. 결과: 40개 이상 파일이 런타임 확인 한 번 없이 작성됐다.

\textbf{설계 원칙}: 핵심 강제 규칙은 에이전트가 실제로 로드하는 파일(SKILL.md)의 처음 100줄에 있어야 한다. 2000줄짜리 참조 파일 깊숙이 묻어두면 안 된다. 참조 파일은 상세 내용을, SKILL.md는 트리거를 제공한다.
\end{warnbox}
...
모듈 시스템(47개 concern, 15개 archetype, 10개 interface, 40개 이상의 foundation, 65개 cross-concern 통합 규칙)은 진정한 도메인 엔지니어링을 나타낸다. 각 모듈은 특정 프로젝트 유형에 무엇이 중요한지에 대한 전문 지식을 인코딩한다---그렇지 않으면 시니어 엔지니어의 머릿속에만 존재하는 지식.
...
시스템은 자체 구축 과정과 참조 프로젝트 시뮬레이션 Pipeline을 통해 개발하고 테스트했다. 실제 개발팀이 참여하는 프로덕션 코드베이스에서 장기 검증은 아직 이루어지지 않았다. 실패 패턴과 대응책은 실전에서 나온 것이지만, 커버리지 주장은 프로덕션 확인이 필요하다.
...
강제 메커니즘(인라인 반복, 차단 게이트, 안티패턴 예시)은 현세대 LLM에 맞춰 교정한 것이다. 모델이 지시를 더 잘 따르게 되면 이 중 일부가 불필요한 오버헤드가 될 수 있고, 반대로 모델이 합리화에 더 능숙해지면 새로운 회피 패턴이 나타나 새 대응책이 필요해질 수도 있다.
...
Domain Profile은 spec-kit-skills의 조합적 타입 시스템이다. 프로젝트의 근본 특성을 다섯 개의 독립 축으로 포착하고, 각 축이 Pipeline 동작을 형성하는 규칙을 기여한다.
...
여러 concern이 활성화되면 어느 단일 모듈도 홀로 생성하지 못하는 창발적 패턴이 발생한다. 이 65개 규칙은 \file{\_resolver.md} Step~3.5에 정의되어 있다:
...
6개 시나리오의 상세 워크스루와 각 시나리오에서 어떤 패밀리가 활성화되는지는 Chapter~\ref{ch:scenarios} \S~전체 시나리오 참조를 참고하세요.
...
\subsection{구체적 로딩 트레이스}

Domain Profile이 \texttt{gui + auth + realtime + ai-assistant + electron + rebuild}인 프로젝트의 경우:
...
CLI 프로젝트(\texttt{cli + resilience + microservice + go})와 비교하면: 완전히 다른 모듈이 로드되어 완전히 다른 규칙이 생성된다. 같은 Pipeline이지만 다른 도메인 지식이 적용되는 것이다.
...
Global Evolution Layer(GEL)는 Feature 간 정보를 전달하는 프로젝트 전역 아티팩트의 집합으로, P1-c(Cross-Feature Memory)의 구현체다.
...
\begin{description}[style=nextline]
  ...
  \item[\file{constitution-seed.md}] \cmd{init} 또는 \cmd{reverse-spec}에서 생성되는 원칙 초안---constitution 확정의 입력물. 버전 관리된다(\file{.specify/memory/constitution.md}는 spec-kit 내부 포맷이므로 직접 편집 불가). 시드를 편집한 후 constitution을 재실행하면 전파된다.
  \item[\file{coverage-baseline.md}] 리빌드/도입 프로젝트를 위한 SBI 커버리지 추적. 소스 행동을 Feature 구현에 매핑하며 P1/P2/P3 우선순위를 부여한다. 의도적 제외 항목은 근거와 함께 기록한다. reverse-spec Phase~4에서 생성된다.
  \item[\file{verify-report.md}] Feature별 런타임 검증 증거. SC별 pass/fail과 Method(runtime/RUNTIME\_BLOCKED/RUNTIME\_DELEGATED), Phase~1--4 결과, 발견된 버그, 머지 준비 판정을 포함한다. 머지 게이트의 BLOCKING 조건. \file{specs/NNN-feature/verify-report.md}에 위치한다.
  \item[\file{demos/F00N-name.sh}] Feature별 실행 가능한 데모 스크립트. 기본 모드는 서버를 시작하고 안내를 표시하며, \texttt{--ci} 모드는 자동화된 검사를 실행한다. implement에서 생성되고 verify Phase~3에서 검증된다.
  \item[\file{demos/DG\{N\}-scenario.sh}] Demo Group 완료 시 통합 데모 스크립트. Cross-Feature 사용자 여정 검증. 마지막 Feature의 implement에서 생성되고 verify Phase~5에서 실행된다. Feature 데모와 동일한 듀얼 모드 구조.
\end{description}
...
각 Feature는 이전 Feature들이 축적한 지식 위에 구축된다. 레지스트리가 Feature마다 풍부해지면서 복리 효과를 낳는다.
...
Part~II에서는 아키텍처를 다루었다: Domain Profile이 규칙을 형성하고, 모듈이 선택적으로 로드되며, 가드가 정확성을 강제하고, GEL이 Cross-Feature 메모리를 보존한다. Part~III에서는 네 스킬이 이 아키텍처를 어떻게 활용하는지 보여준다. \texttt{code-explore}가 진입점으로서 오리엔테이션 아티팩트와 Feature 후보를 생성하면, \texttt{reverse-spec}이 이를 정제하고 \texttt{smart-sdd}가 소비한다. \texttt{domain-extend}는 나머지 세 스킬이 활용하는 어휘를 확장한다. 모든 스킬은 Chapter~8에서 설명한 동일한 GEL 아티팩트를 읽고 쓴다.
...
\cmd{code-explore}는 이해를 위한 스킬이다. 사용자가 직접 안내하는 인터랙티브 소스 코드 탐색으로 영속적이고 구조화된 문서를 생성한다.
...
\section{Learn 모드 (\texttt{--learn})}

이미 구현되고 SDD로 관리되는 프로젝트에서 \texttt{--learn}은 새 Feature 후보를 생성하거나 기존 파이프라인 상태와 충돌하지 않는 읽기 전용 탐색을 제공합니다.

\begin{description}
  \item[목적] 기존 프로젝트의 아키텍처와 코드 구조 이해
  \item[출력] \file{specs/explore/learn/synthesis.md} --- 아키텍처 요약 + 기존 Feature 맵
  \item[생성하지 않는 것] Feature 후보 (C001\ldots), Domain Profile 추론, 핸드오프 준비
  \item[포함] \texttt{--no-branch} (git 브랜치 생성 안 함)
  \item[사용 시점] 팀 온보딩, 아키텍처 리뷰, 감사, 미팅 준비
\end{description}

\begin{tipbox}[기본 모드 vs Learn 모드]
기본 모드: ``어떤 Feature를 만들어야 하나?'' Learn 모드: ``이미 만든 것이 어떻게 동작하나?'' 파이프라인 전에는 기본 모드, 파이프라인 후에는 \texttt{--learn}을 사용합니다.
\end{tipbox}
...
모든 Pipeline 단계는 동일한 프로토콜을 따른다:

\begin{enumerate}
  \item \textbf{Assemble}: Domain Profile 모듈, GEL 아티팩트, pre-context를 로드한다. Cross-Feature 메모리가 활성화되는 지점이다.
  \item \textbf{Checkpoint (HARD STOP)}: 조립된 컨텍스트를 표시하고 사용자 승인을 대기한다.
  \item \textbf{Execute + Review}: 주입된 컨텍스트와 함께 spec-kit 명령을 실행한다. raw 출력을 억제하고, 생성된 아티팩트를 읽어 포맷된 리뷰를 표시한 뒤 AskUserQuestion을 호출한다 (HARD STOP).
  \item \textbf{Update}: 새 entity/API를 GEL에 등록하고 \file{sdd-state.md}를 갱신한다.
\end{enumerate}

\begin{dangerbox}[필수 규칙]
여덟 가지 필수 규칙이 항상 SKILL.md에 로드됩니다:
...
\textbf{규칙~4}: Feature는 순차 실행. 여러 Feature를 병렬로 처리하지 않습니다.

\textbf{규칙~5}: 런타임 SC 검증 필수. build+test 통과는 verify 완료가 아닙니다.

\textbf{규칙~6}: 정직한 SC 증거. 부분 검증 = PARTIAL (PASS 아님). User Demo 필수.

\textbf{규칙~7}: Implement Completeness Gate. Review 전: 모든 task 완료 + 데모 스크립트 존재 + smoke launch 통과.

\textbf{규칙~8}: Spec 없이 코드 금지. verify에서 누락 기능 발견 시, spec.md를 먼저 업데이트(Cascading Update Protocol)한 후 구현. FR/SC가 없는 코드를 절대 작성하지 않는다.
\end{dangerbox}
...
6개 Phase는 다음과 같다:
\begin{enumerate}[nosep]
  \item \textbf{Phase 1: Briefing}---입력을 파싱하고, 관점 격차를 식별하며, S5/A3 프로브로 정교화
  \item \textbf{Phase 2: Overlap \& Impact}---기존 Feature와의 중복을 확인하고, GEL 영향을 평가
  \item \textbf{Phase 3: Scope Negotiation}---모호함을 해소하고, In-Scope/Out-of-Scope 경계를 확정
  \item \textbf{Phase 4: SBI Match}---Source Behavior Inventory에 매핑(브라운필드), 커버리지 확인
  \item \textbf{Phase 5: Demo Group}---Integration Demo 조율을 위해 Demo Group에 할당
  \item \textbf{Phase 6: Finalization}---아티팩트 생성 (pre-context, sdd-state 업데이트, add-draft)
\end{enumerate}
...
\begin{description}
  ...
  \item[Phase 3: UI/런타임 검증] Playwright를 통해 애플리케이션을 실행합니다. 실행 중인 UI에서 각 SC를 검증합니다. Playwright를 사용할 수 없으면 사용자에게 위임합니다---절대 건너뛰지 않습니다. (서버 기동은 활성 Foundation의 F8b Runtime Environment 필드를 사용합니다---Chapter~5 \S~F8b 참조.)
...
사용자가 HARD STOP에서 피드백을 제공하면, 단순히 이전 단계로 돌아가는 것뿐 아니라 하류 영향을 분석하고 다음 파이프라인 액션을 제안해야 하는 경우가 많습니다. 다음 섹션에서 이를 Impact Analysis + Flow Proposal 프로토콜로 공식화합니다.

\section{Impact Analysis + Flow Proposal}

파이프라인의 모든 HARD STOP (Checkpoint 또는 Review)에서 사용자는 버그, 누락된 요구사항, 개선 아이디어에 대한 피드백을 제공할 수 있습니다. 에이전트는 이슈를 분석하는 데 그치지 않고 피드백을 분류하고, 하류 영향을 평가하며, 구체적인 파이프라인 액션을 제안해야 합니다---사용자가 직접 라우팅을 파악할 필요가 없도록.
...
\begin{description}
  ...
  \item[\_resolver.md] Cross-Concern 통합 (Step~3.5, 65개 규칙), 스케일 조정 (Step~4), 지연 로딩 섹션 테이블 (Step~5)을 포함하는 7단계 모듈 로딩 프로토콜.
...
\begin{tipbox}[내 프로젝트에 맞는 모듈 찾기]
47개 Concern을 전부 읽는 대신, 프로젝트 유형에서 출발하자:

\textbf{AI 어시스턴트를 만드시나요?} $\rightarrow$ \texttt{ai-assistant} archetype + \texttt{gui} + \texttt{realtime} + \texttt{external-sdk} + \texttt{auth}

\textbf{REST API를 만드시나요?} $\rightarrow$ \texttt{public-api} 또는 \texttt{microservice} archetype + \texttt{http-api} + \texttt{auth} + \texttt{resilience}

\textbf{CLI 도구를 만드시나요?} $\rightarrow$ \texttt{cli} interface + 관련 concern (단순 도구라면 archetype 불필요)

\textbf{레거시 코드를 재작성하시나요?} $\rightarrow$ \texttt{reverse-spec}이 소스 코드에서 모듈을 자동 감지합니다. 직접 선택할 일이 거의 없습니다.

\textbf{잘 모르겠다면?} $\rightarrow$ \texttt{/smart-sdd init}에 프로젝트 설명을 입력하세요. CI 스코어링 시스템이 Domain Profile을 추론하고 어떤 모듈이 활성화되는지 보여줍니다.
\end{tipbox}
...
\textit{이 챕터는 특정 모듈 조합이 함께 로드될 때 활성화되는 65개 통합 규칙을 설명한다. 이 규칙들은 개별 모듈에는 포함되지 않는 창발적 동작을 생성한다.}
...
\section{근본적 한계: Pipeline Completion Bias는 규칙으로 해결할 수 없다}

이 챕터의 모든 대응책은 하나의 전제를 공유한다: \textbf{에이전트가 규칙을 읽고 따를 것이다.} aegis 파일럿은 이 전제가 장시간 파이프라인 실행 후 체계적으로 무너진다는 것을 보여주었다.

\begin{dangerbox}[이 매뉴얼에서 가장 중요한 발견]
2시간 이상의 연속 파이프라인 실행(specify $\rightarrow$ plan $\rightarrow$ tasks $\rightarrow$ analyze $\rightarrow$ implement) 후, 에이전트는 암묵적 최적화 목표를 발전시킨다: \textbf{파이프라인을 끝내는 것.} 이 ``Pipeline Completion Bias''는 에이전트가 읽고 이해했음을 증명할 수 있는 명시적 규칙까지 무력화한다.

\textbf{aegis 파일럿 증거} (F007--F008):
\begin{itemize}[nosep]
  \item SKILL.md Rule~5 (런타임 SC 검증): 세션 시작 시 읽었으나 verify에서 위반
  \item SKILL.md Rule~7 (Per-Task Micro-Verify): 세션 시작 시 읽었으나 implement에서 위반
  \item injection/implement.md: pipeline.md:1333에 ``반드시 읽으라''고 되어 있지만 로드하지 않음
  \item SC Matrix Pre-population: BLOCKING으로 정의됐으나 실행하지 않음
  \item verify-report Match? 컬럼: 채웠지만 ``CODE'' 결과를 $\checkmark$ PASS로 표기
\end{itemize}

\textbf{패턴}: 모든 규칙이 존재했고, 접근 가능했고, 인정까지 했다. 하지만 시간 압박 하에서 에이전트는 최소 저항 경로를 찾았다: 빠르게 코드 작성, 빌드 한 번, 전부 통과로 보고. 규칙을 더 추가해도 이것은 고쳐지지 않는다 --- 에이전트는 이미 가진 규칙도 무시하고 있다.
\end{dangerbox}

\paragraph{왜 스킬 설계만으로는 해결할 수 없는가.}

이것은 \textbf{모델 수준의 행동}이지 스킬 수준의 버그가 아니다. 에이전트의 암묵적 목표(효율적으로 작업 완료)가 명시적 지시(모든 SC를 런타임에서 검증)와 충돌할 때, 둘 다 같은 유한한 자원(컨텍스트 윈도우와 시간)을 필요로 한다. 자원 압박 하에서 암묵적 목표가 이긴다.

인간 소프트웨어 엔지니어링의 비유: 12시간 동안 코딩한 개발자는 테스트를 줄인다 --- 테스트가 중요한 줄 몰라서가 아니라, 피로가 비용-편익 계산을 바꾸기 때문이다. 어떤 프로세스 문서도 이것을 막지 못한다. 막는 것은 \textbf{구조적 분리}: 개발자가 코드를 쓰고, 다른 사람이 리뷰하고, CI가 테스트를 돌린다.

\paragraph{실전 완화책.}

에이전트 준수 여부와 관계없이 동작하는 세 가지 구조적 접근:

\begin{enumerate}
  \item \textbf{세션 분리}: implement와 verify는 반드시 다른 세션에서 수행. implement 완료 후 \texttt{/clear}, 새 세션에서 \texttt{/smart-sdd pipeline F00N --start verify}. verify 세션에는 implement 피로, 매몰 비용, ``코드가 동작하는 걸 이미 안다''는 편향이 없다. \textbf{가장 효과적인 단일 완화책.}

  \item \textbf{사용자가 최종 게이트}: User Demo Gate (MANDATORY RULE~6)는 에이전트의 자기 검증이 신뢰할 수 없기 때문에 존재한다. 사용자가 실제로 동작하는 애플리케이션을 봐야 한다.

  \item \textbf{verify-report는 영속적 증거}: 에이전트가 모든 것을 PASS로 표시해도, verify-report가 파일로 존재한다. 사용자가 읽어보면 8개 SC 중 7개가 ``METHOD: CODE''인 것을 바로 알 수 있다. 리포트는 위반을 막지 못하지만 \textbf{위반을 보이게, 감사 가능하게} 만든다.
\end{enumerate}

\begin{tipbox}[솔직한 권고]
프로덕션 프로젝트에 spec-kit-skills를 쓴다면: \textbf{같은 세션에서 implement와 verify를 절대 돌리지 마라.} 둘 사이에 컨텍스트를 비워라. implement 결과를 ``테스트했다''고 말하지만 PR에 테스트 파일이 없는 주니어 개발자의 PR과 같은 수준으로 의심하라. 에이전트는 거짓말하는 게 아니다 --- 진심으로 검증했다고 믿는다. 하지만 믿음은 증거가 아니다.
\end{tipbox}
...
\textit{이 섹션은 6가지 공식 시나리오 각각의 정의, 예시, 4-family 활용 테이블, 핵심 특징을 상세 참조용으로 제공한다.}

spec-kit-skills는 6가지 시나리오를 공식 지원한다. 각 시나리오는 프로젝트의 출발점과 목표에 따라 4개 스킬(code-explore, reverse-spec, smart-sdd, domain-extend)을 서로 다른 조합으로 활용한다. 이 섹션에서는 각 시나리오의 정의, 예시, 4-family 활용 테이블, 핵심 특징을 상세히 다룬다.


%% ── 시나리오 1: Greenfield ──────────────────────────────────────
\subsection{시나리오 1: Greenfield --- 처음부터 새로 만들기}
...
\subsection{시나리오 2: Rebuild (Same Stack) --- 같은 기술로 재작성}
...
\subsection{시나리오 3: Rebuild (New Stack) --- 다른 기술로 재작성}
...
%% ── 시나리오 4: Incremental ─────────────────────────────────────
\subsection{시나리오 4: Incremental --- 기존 SDD 프로젝트에 기능 추가}
...
%% ── 시나리오 5: Adoption ────────────────────────────────────────
\subsection{시나리오 5: Adoption --- 기존 코드에 SDD 문서화}
...
%% ── 시나리오 6: +migration ──────────────────────────────────────
\subsection{시나리오 6: +migration --- 기술 전환}
...
\section{Cross-Feature API 갭 해소}

Feature가 이미 merge된 이전 Feature에서 제공해야 할 API를 필요로 할 때, 세 가지 옵션이 있습니다:

\begin{description}
  \item[옵션 A: 현재 Feature에서 구현] 누락된 API를 \emph{현재} Feature에 추가. spec.md에 ``보조 구현 (F003 scope gap)''으로 문서화. api-registry에 실제 provider 갱신. \textbf{개별 엔드포인트에 권장.}
  \item[옵션 B: 이전 Feature 재개] \texttt{add --to F003}으로 전체 re-pipeline (specify $\rightarrow$ plan $\rightarrow$ tasks $\rightarrow$ implement $\rightarrow$ verify $\rightarrow$ merge). \textbf{아키텍처 변경에만 권장} --- 엔드포인트 하나에는 과잉.
  \item[옵션 C: RUNTIME\_BLOCKED 표시] API가 없어서 SC 검증 불가. 향후 Feature 또는 Tier로 연기.
\end{description}

\begin{tipbox}[결정 휴리스틱]
\textbf{개별 엔드포인트 누락?} $\rightarrow$ 옵션 A (현재 Feature에서 구현, spec에 문서화).\\
\textbf{데이터 모델 또는 아키텍처 변경 필요?} $\rightarrow$ 옵션 B (이전 Feature 재개).\\
\textbf{전체 서브시스템 누락?} $\rightarrow$ 옵션 C (새 Feature로 연기).

merge된 Feature를 재개하는 비용(6+ 파이프라인 단계)이 변경을 정당화해야 합니다. PATCH 엔드포인트 하나가 전체 re-pipeline을 정당화하지 않습니다.
\end{tipbox}

Feature Boundary Clarification Gate(specify에 추가됨)가 이것을 spec 시점에 잡습니다: ``API가 registry에 없음 $\rightarrow$ In-Scope에 추가할까요, Out-of-Scope로 할까요?'' verify 시점에 발견되면(spec이 gate 이전에 만들어진 경우) 옵션 A가 현실적 해결책입니다.
...
\textit{\speckit{}으로 4일간 구축한 완전한 그린필드 프로젝트: 12개 Feature, 122개 Success Criteria, 3개 커스텀 도메인 모듈, 그리고 프레임워크의 9개 MANDATORY RULE을 이끌어낸 22건의 스킬 피드백.}

%% ── 프로젝트 개요 ───────────────────────────────────────────────────
\section{프로젝트 개요}

\textbf{AEGIS} (AI Enterprise Gateway \& Intelligence System)는 LLM 라우팅, 멀티테넌트 비용 관리, 보안 가드레일, 지식 통합을 하나의 셀프호스팅 오픈소스 플랫폼으로 통합하는 엔터프라이즈 AI 게이트웨이입니다.

\begin{table}[H]
\centering
\caption{AEGIS 프로젝트 요약}
\label{tab:aegis-summary-ko}
\begin{tabularx}{\textwidth}{lX}
\toprule
\textbf{항목} & \textbf{값} \\
\midrule
유형 & 그린필드 \\
기간 & 4일 (2026-03-25 ~ 2026-03-28) \\
기술 스택 & TypeScript, NestJS (백엔드), Next.js + shadcn/ui (프론트엔드), PostgreSQL + pgvector, Redis, BullMQ \\
Feature 수 & 12개, 4개 Tier (T0--T3), 4개 Release Group \\
Success Criteria & 총 122개 \\
테스트 & 185개 (Jest 단위 + 통합) \\
소스 파일 & $\sim$120개 (apps/api + apps/web + libs/common) \\
커스텀 도메인 모듈 & 3개 (ai-gateway, token-budget, prompt-guard) \\
스킬 피드백 & 22건 (P1--P18, F007 P8--P11, F008 P12--P13) \\
\bottomrule
\end{tabularx}
\end{table}

%% ── 도메인 프로파일 ─────────────────────────────────────────────────
\section{도메인 프로파일과 커스텀 모듈}

\subsection{5축 프로파일}

\cmd{smart-sdd init}에서 설정된 프로젝트 도메인 프로파일. Clarity Index 97\% (Rich 티어):
...
# * = 프로젝트 로컬 커스텀 모듈 (내장 라이브러리에 없음)
\end{lstlisting}

\subsection{갭 분석과 모듈 생성}

프로젝트 설명을 내장 택소노미(58개 Concern, 14개 Archetype, 10개 Interface)와 대조한 결과 3개 갭 발견:

\begin{table}[H]
\centering
\caption{도메인 모듈 갭 분석}
\label{tab:aegis-gaps-ko}
\begin{tabularx}{\textwidth}{llcX}
\toprule
\textbf{갭} & \textbf{가장 유사한 내장 모듈} & \textbf{유사도} & \textbf{새 모듈이 필요한 이유} \\
\midrule
AI Gateway 라우팅 & ai-assistant & 15\% & 내장 모듈은 LLM \emph{소비자} 관점; AEGIS는 LLM \emph{관리자} 관점 필요 \\
계층적 토큰 예산 & resilience & 20\% & 단순 속도 제한 vs Org$>$Team$>$User 계층적 토큰+비용 추적 \\
프롬프트 보안 & content-moderation & 25\% & 비동기 리뷰 큐 vs 인라인 $<$100ms LLM 특화 스캐닝 \\
\bottomrule
\end{tabularx}
\end{table}

\cmd{domain-extend extend}로 3개 프로젝트 로컬 모듈 생성:

\begin{itemize}[nosep]
  \item \textbf{ai-gateway} (Archetype) --- 5개 원칙 (Provider Abstraction, Streaming-First, Token-Aware Routing, Multi-tenant Isolation, Audit Everything), 5개 장애 모드 (AG-001--005)
  \item \textbf{token-budget} (Concern) --- SC 필수 패턴 5개, 안티패턴 4개, 장애 모드 5개 (TB-001--005), 정교화 프로브 6개
  \item \textbf{prompt-guard} (Concern) --- SC 필수 패턴 5개, 안티패턴 4개, 장애 모드 5개 (PG-001--005), 정교화 프로브 6개
\end{itemize}

\begin{notebox}[커스텀 모듈의 영향]
이 모듈들은 파이프라인의 모든 단계에 참여했습니다: S0 키워드는 \cmd{init}의 시그널 추출 정확도를 높였고, S1 규칙은 \texttt{specify}에서 SC 패턴을 강제했으며, S5 프로브는 \cmd{add}의 Brief를 심화했고, S7 장애 모드는 \texttt{plan}과 \texttt{implement}의 방어적 설계를 안내했습니다. 이 모듈 없이는 22개 SC 규칙과 15개 장애 모드가 에이전트의 임의 판단에 맡겨졌을 것입니다.
\end{notebox}

%% ── 파이프라인 실행 ─────────────────────────────────────────────────
\section{파이프라인 실행}

\subsection{Feature별 결과}

\begin{table}[H]
\centering
\caption{Feature별 파이프라인 결과}
\label{tab:aegis-features-ko}
\small
\begin{tabularx}{\textwidth}{clXcccc}
\toprule
\textbf{FID} & \textbf{Tier} & \textbf{이름} & \textbf{SC} & \textbf{Verify} & \textbf{테스트} & \textbf{세션} \\
\midrule
...
\midrule
\multicolumn{3}{l}{\textbf{합계}} & \textbf{122} & \textbf{8 PASS / 4 LTD} & \textbf{185} & \textbf{16} \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Verify 수준}: 8개 Feature가 완전한 런타임 검증(실행 중인 서버에 curl + Playwright)을 달성. 4개 Feature(F004 일부, F010--F012)는 인프라 제약(DB/Redis 미가동)으로 코드 수준 검증만 수행. 코드 수준 검증으로도 F011 Cross-Feature 회귀(SC-011)를 포함한 구조적 결함 발견.

\subsection{Demo Group: Cross-Feature 통합 검증}

T1 완료 후 두 개의 Demo Group으로 엔드투엔드 사용자 시나리오 검증:

\begin{itemize}[nosep]
  \item \textbf{DG1} (F002+F003+F004): JWT 로그인 $\to$ LLM 호출 $\to$ 예산 차감 $\to$ 예산 소진 $\to$ 429 차단. 8/8 SC PASS.
  \item \textbf{DG2} (F003+F005): API Key 인증 $\to$ Trace ID 전파 LLM 호출 $\to$ 비동기 로그 생성 $\to$ 분석 쿼리. 8/8 SC PASS.
\end{itemize}

\subsection{회귀 발견 및 수정}

\begin{table}[H]
\centering
\caption{회귀 사례}
\label{tab:aegis-regressions-ko}
\begin{tabularx}{\textwidth}{clXX}
\toprule
\textbf{Feature} & \textbf{SC} & \textbf{근본 원인} & \textbf{해결} \\
\midrule
F004 & SC-003 & 토큰 추정이 입력 콘텐츠만 계산; 출력 토큰 완전 누락 & 비관적 추정 공식: input/3 + 메시지 오버헤드 + max\_tokens 폴백 \\
F004 & SC-014 & \texttt{extractUserKey()}가 null 반환 --- reconciliation 미실행 & Redis reservation HASH에 키 경로 저장; reconcile이 경로 직접 읽기 \\
F011 & SC-011 & CacheInterceptor가 \texttt{request.cacheHit} 플래그 미설정 & F005 RequestLoggerInterceptor가 항상 \texttt{cacheHit:~false} 기록; 2개 파일 수정 \\
\bottomrule
\end{tabularx}
\end{table}

F004 회귀는 전체 파이프라인 재실행(\texttt{--start specify})을 트리거하여 spec$\to$plan$\to$tasks$\to$implement$\to$verify 캐스케이딩. F011은 타겟 implement 회귀로 해결.

%% ── 핵심 순간들 ─────────────────────────────────────────────────────
\section{핵심 순간들}

\subsection{HARD STOP 개입}

4건의 핵심 발견이 \hardstop{} 리뷰 시점에서만 이루어짐:

\begin{table}[H]
\centering
\caption{\hardstop{} 개입 사례}
\label{tab:aegis-hardstop-ko}
\begin{tabularx}{\textwidth}{clXX}
\toprule
\textbf{\#} & \textbf{단계} & \textbf{발생 사항} & \textbf{\hardstop{} 없었다면} \\
\midrule
1 & F004 specify & UsageRecord 상태값 불일치 (\texttt{failed} vs \texttt{released}) 리뷰에서 포착 & 잘못된 상태 머신 구현; 런타임 데이터 손상 \\
2 & F004 specify & 사용자가 F001--F003 산출물 한국어 전환 요청 & 전환 기회 놓침; implement 이후 대규모 재작업 \\
3 & F004 verify & 사용자 도메인 판단 주입: ``모델별 예산 관리(ModelTier) 필요'' & 모델 무관 예산으로 완성; 추후 대규모 리팩토링 \\
4 & F003 verify & Refresh token jti 충돌 버그 발견 & 운영 환경에서 1초 내 2회 로그인 시 토큰 충돌 \\
\bottomrule
\end{tabularx}
\end{table}

\begin{dangerbox}[핵심 통찰]
\hardstop{}은 단순한 승인 게이트가 아닙니다. 도메인 전문 지식이 파이프라인에 진입할 수 있는 \emph{유일한} 시점입니다. 개입 \#3 --- ModelTier 예산 요구사항 --- 은 코드 분석으로 도출할 수 없으며, 사용자만이 가진 비즈니스 지식에서 나왔습니다.
\end{dangerbox}

\subsection{병렬 실행 중단 (P3)}

프로젝트 초기에 에이전트가 F003, F004, F005를 백그라운드 에이전트로 동시 처리 시도. MANDATORY RULE~4 (순차적 Feature 실행) 위반으로 즉시 중단. 파일 충돌, 레지스트리 참조 불일치, 엔티티 중복이 발생하여 규칙의 존재 이유를 확인.

\subsection{Pipeline Completion Bias 증거}

F010--F012는 긴 세션 후반부에 실행되었으며, 런타임 검증 가능한 SC가 있음에도 모두 코드 수준 검증만 받음. 에이전트의 ``빨리 끝내야 한다''는 암묵적 압력이 다음과 같이 발현:

\begin{itemize}[nosep]
  \item Level~3 (CRUD) SC에 대해 Level~1 (렌더링만) 검사 수행
  \item ``명확한'' 프론트엔드 페이지에 대해 Playwright 생략
  \item ``페이지가 정상 렌더링됩니다''를 ``SC 완료''로 보고
\end{itemize}

이 패턴이 MANDATORY RULE~9 (세션 분리)를 직접 이끌어냄: implement와 verify는 다른 세션에서 실행하여 누적 피로가 검증 엄밀성을 저하시키는 것을 방지.

%% ── 스킬 피드백 진화 ────────────────────────────────────────────────
\section{스킬 피드백 진화}

22건의 스킬 피드백은 예측 가능한 4단계 패턴을 따름:

\begin{table}[H]
\centering
\caption{단계별 스킬 피드백 진화}
\label{tab:aegis-feedback-ko}
\begin{tabularx}{\textwidth}{llcX}
\toprule
\textbf{단계} & \textbf{항목} & \textbf{심각도} & \textbf{패턴} \\
\midrule
초기 (F001--F003) & P1--P4 & CRITICAL & 기본 위반: 템플릿 미준수, verify 스킵, 병렬 실행 \\
중기 (F004--F005) & P5--P7 & HIGH & Verify 품질 저하: 런타임 검사 우회, 부분 SC 커버리지 \\
후기 (F006--F009) & P8--P14 & HIGH--MED & 세부 갭: npm install 누락, 렌더링$=$PASS 오류, micro-verify 부재 \\
통합 & P15--P18 & MEDIUM & 환경 및 UX 이슈: .env 처리, 데모 스크립트 가이드 \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{피드백에서 MANDATORY RULE로}

프레임워크의 9개 MANDATORY RULE이 AEGIS 피드백에서 직접 도출:

\begin{table}[H]
\centering
\caption{피드백-규칙 추적성}
\label{tab:aegis-rules-ko}
\begin{tabularx}{\textwidth}{lXl}
\toprule
\textbf{피드백} & \textbf{문제} & \textbf{생성된 규칙} \\
\midrule
P2 & Verify 단계 전체 스킵 & Rule 5: 런타임 SC 검증 \\
P3 & Feature 병렬 처리 & Rule 4: 순차적 Feature 실행 \\
P5, P6 & 단위 테스트 통과를 SC 검증으로 간주 & Rule 5 (강화) \\
P9 & \hardstop{} 리뷰 우회 & Rule 1: \hardstop{} 강제 \\
P10, P11 & 렌더링을 PASS로 간주; spec 없이 코드 추가 & Rule 6: 정직한 SC 증거 \\
P11-D & Spec 없이 코드 추가 & Rule 8: Spec 없이 코드 금지 \\
P12, P13 & 코드 일괄 작성; 코드 수준 PASS 보고 & Rule 7: Per-Task Micro-Verify \\
F010--F012 & 후반부 세션 verify 품질 저하 & Rule 9: 세션 분리 \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Micro-Verify: 대조 실험}

F008과 F009는 Per-Task Micro-Verify의 자연스러운 A/B 비교 사례:

\begin{itemize}[nosep]
  \item \textbf{F008 (미적용)}: 서킷 브레이커 코드 전체를 일괄 작성 $\to$ P12 + P13 피드백; 문제가 verify에서야 발견
  \item \textbf{F009 (적용)}: 각 태스크 작성 직후 즉시 검증 $\to$ 3건 런타임 에러를 implement 중 포착·수정 (BullMQ 연결, pgvector 타입 충돌, SQL 컬럼 케이싱)
\end{itemize}

이 대조가 MANDATORY RULE~7의 Per-Task Micro-Verify 요구사항을 직접 이끌어냄.
...
\centering
\caption{AEGIS 최종 지표}
\label{tab:aegis-metrics-ko}
\begin{tabularx}{\textwidth}{Xr}
\toprule
\textbf{지표} & \textbf{값} \\
\midrule
완료 Feature & 12 / 12 \\
Success Criteria (총) & 122 \\
런타임 검증 Feature & 8 / 12 (67\%) \\
코드 수준 검증 Feature & 4 / 12 (33\%) \\
Jest 테스트 (최종) & 185 (26개 스위트) \\
소스 파일 & $\sim$120 \\
커스텀 도메인 모듈 & 3 \\
스킬 피드백 항목 & 22 \\
회귀 발견 / 수정 & 3 \\
\hardstop{} 개입 & 4 \\
도출된 MANDATORY RULE & 9 \\
Demo Group 검증 & 2 (16/16 SC) \\
파이프라인 세션 (총) & 16 \\
소요 일수 & 4 \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}[nosep]
  \item \textbf{Verify $\neq$ 빌드 + 테스트 통과.} 실행 중인 서버에 대한 런타임 SC 검증만이 유효한 증거. 단위 테스트 통과는 전제 조건이지 결론이 아님 (P2, P5, P6).
  \item \textbf{\hardstop{}은 대체 불가.} 코드에서 도출할 수 없는 도메인 지식(ModelTier 예산, 언어 선호, 상태 머신 수정)은 사용자 리뷰를 통해서만 진입 가능 (P9, \S\ref{tab:aegis-hardstop-ko}).
  \item \textbf{Feature는 반드시 순차 실행.} 병렬 실행은 entity-registry 충돌, Cross-Feature 참조 불일치, 추적 불가능한 상태 손상을 유발 (P3).
  \item \textbf{Per-Task Micro-Verify가 버그를 더 일찍 잡는다.} F008 (미적용) vs F009 (적용)로 일괄 검증이 문제를 누적시키고 태스크별 검증이 격리한다는 것을 입증 (P12, P13).
  \item \textbf{프론트엔드와 백엔드는 다른 검증 전략 필요.} 백엔드: API 엔드포인트에 curl. 프론트엔드: Playwright 전체 경로 검증 + API smoke test + npm install 게이트 (F007 P8--P11).
  \item \textbf{Cross-Feature 이슈는 verify에서 발견된다.} F011 SC-011 회귀(CacheInterceptor $\to$ RequestLogger 데이터 흐름)는 Per-Task Micro-Verify로는 보이지 않았지만 verify Phase~2에서 포착.
  \item \textbf{커스텀 도메인 모듈은 투자 대비 효과가 크다.} ai-gateway, token-budget, prompt-guard가 기여한 22개 SC 규칙과 15개 장애 모드가 결함 범주 전체를 verify 이전에 차단.
  \item \textbf{Pipeline Completion Bias는 실재하며 구조적이다.} 2시간 이상의 파이프라인 실행 후 verify 품질이 측정 가능하게 저하. 유일한 해결책은 세션 분리 (MANDATORY RULE~9).
  \item \textbf{Spec 없이 코드 금지.} verify에서 누락 기능 발견 시 수정은 코드가 아닌 spec에서 시작. 캐스케이딩 업데이트(spec $\to$ plan $\to$ tasks $\to$ implement $\to$ verify)가 SDD 무결성 보존 (P11-D, Rule~8).
  \item \textbf{보정 메커니즘이 정밀도보다 먼저.} F004의 reconciliation 버그는 보정이 작동하는 부정확한 추정이 보정이 죽은 정확한 추정보다 낫다는 것을 가르침.
\end{enumerate}
...
\begin{description}[style=nextline]
  \item[Brief] 모든 입력을 일관되게 완전한 Feature 정의로 정규화하는 구조화된 접수 프로세스. PRD와 다르다---Brief는 완전성과 정확성이 검증된 상담의 \emph{출력}이다.

  \item[BLOCKING gate] Pipeline 진행을 가로막는 구조적 장벽. 경고나 가이드라인과 달리, 게이트된 단계를 완료하지 않으면 에이전트가 다음 출력을 물리적으로 생성할 수 없다.

  \item[Context Injection] 각 Pipeline 단계 전에 관련 도메인 모듈, GEL 아티팩트, Feature 컨텍스트를 조립하는 프로세스. spec-kit-skills의 아키텍처적 핵심이다.

  \item[Cross-Concern Integration Rules] 여러 도메인 모듈이 동시에 로드될 때 활성화되는 창발 패턴. \file{\_resolver.md} Step~3.5에 정의된다.
  ...
  \item[Feature] 독립적으로 명세, 구현, 검증할 수 있는 자기 완결적 기능 단위. 각 Feature는 정확히 하나의 spec, 하나의 plan, 하나의 구현 사이클을 갖는다.

  \item[GEL (Global Evolution Layer)] Feature 간에 정보를 전달하는 프로젝트 전체 아티팩트 집합 (레지스트리, 상태, 로드맵, constitution, pre-context).

  \item[Guard] 실패 유형을 다루는 일반화된 Pipeline 무결성 패턴. 현장에서 발견된 44건의 사건에서 추출한 7개 guard가 정의되어 있다.

  \item[HARD STOP] Pipeline 실행이 일시 정지되어 AskUserQuestion을 통한 명시적 사용자 입력을 요구하는 지점. 응답 없이는 진행할 수 없다.

  \item[Harness Engineering] AI 에이전트의 행동을 신뢰할 수 있는 결과로 유도하는 구조화된 시스템을 구축하는 분야. spec-kit-skills의 핵심 테제다.

  \item[Inline Execution] Skill tool 호출 대신 오케스트레이터의 턴 안에서 서브 스킬의 지시를 직접 실행하는 방식 (SKILL.md를 읽고 따름). 제어 흐름 연속성을 보존한다.
