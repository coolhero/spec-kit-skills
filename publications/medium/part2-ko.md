# 네 가지 스킬, 하나의 파이프라인: code-explore, reverse-spec, smart-sdd, domain-extend의 협업

## Part 2 of 4 — 각 스킬 상세

![2편 커버](https://raw.githubusercontent.com/coolhero/spec-kit-skills/main/articles/medium/part2.png)

*1편에서 이어집니다: [AI 코더 길들이기: 왜 에이전트에겐 프롬프트가 아니라 하네스가 필요한가](https://medium.com/@thejihoonchoi/ai-%EC%BD%94%EB%94%A9-%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8-%EA%B8%B8%EB%93%A4%EC%9D%B4%EA%B8%B0-%ED%94%84%EB%A1%AC%ED%94%84%ED%8A%B8%EA%B0%80-%EC%95%84%EB%8B%88%EB%9D%BC-%ED%95%98%EB%84%A4%EC%8A%A4%EA%B0%80-%ED%95%84%EC%9A%94%ED%95%9C-%EC%9D%B4%EC%9C%A0-dcdd752d2e8b)*

---

## /code-explore — 빌드 전에 이해하라

대부분의 개발자는 AI 프로젝트를 "X를 만들어줘"로 시작합니다. 하지만 최고의 프로젝트는 "먼저 Y를 이해하자"에서 시작하죠.

새로운 아이디어는 아닙니다. 모든 시니어 엔지니어가 하는 일이니까요 — 코드를 읽고, 다이어그램을 그리고, 질문을 던진 후에야 한 줄의 코드를 씁니다. 그런데 AI 에이전트를 쓸 때는 이 단계를 건너뛰게 됩니다. 바로 "만들어"로 넘어가고 에이전트가 알아서 컨텍스트를 파악하길 바라죠.
...
메인 이벤트입니다 — Feature 간 메모리를 갖춘 완전한 Specification-Driven Development 파이프라인.
...
대부분의 가치가 이 단계에 있습니다. "인증 추가해줘"가 아니라, 모호한 아이디어를 정확한 Feature 정의로 만들어가는 구조화된 상담을 거칩니다.
...
1. **파이프라인 실행 중 발견.** specify나 plan 단계에서 패턴 갭을 만나면 — 예를 들어 프로젝트가 WebRTC 시그널링을 사용하는데 기존 concern 모듈이 커버하지 않는 경우 — domain-extend가 표준 섹션(S0 시그널 키워드, S1 SC 생성 규칙, S5 정교화 프로브, S7 버그 방지)을 갖춘 새 `webrtc-signaling.md` concern 모듈을 만들어줍니다.

2. **기존 문서에서 임포트.** 팀의 ADR, 스타일 가이드, 포스트모템 보고서에는 힘들게 얻은 도메인 지식이 담겨 있습니다. Domain-extend가 이를 모듈 형식으로 임포트합니다.

3. **검증과 통합.** 새 모듈은 스키마(`_schema.md`)에 대해 검증되고, 섹션 번호 일관성이 체크되며, 선택적으로 resolver의 Cross-Concern Integration 규칙에 대해 테스트됩니다.

핵심 제약: domain-extend는 내장 모듈과 동일한 규칙을 따르는 모듈을 생성합니다. resolver가 자동 발견하고, append 의미론으로 기존 모듈과 병합되며, Cross-Concern Integration에 참여합니다. 생성된 후에는 시스템에 기본 탑재된 모듈과 구별할 수 없습니다.

중요한 점: domain-extend가 만든 모듈은 스킬 설치 디렉토리가 아닌 프로젝트 디렉토리(`specs/domains/`)에 저장됩니다. `git clone`으로 팀원이 바로 받을 수 있고, 다른 프로젝트에 영향을 주지 않으며, spec-kit-skills를 업데이트해도 커스텀 모듈이 덮어쓰이지 않습니다. 공유 설치에 모듈을 기여하려면 `--skill` 플래그를 사용합니다.

여기서 복리 효과가 생깁니다. Feature 1의 파이프라인이 rate-limiting 갭을 발견하고 domain-extend가 `rate-limiting.md` concern 모듈을 만들면, Feature 2부터는 자동으로 그 SC 규칙, 버그 예방 패턴, 정교화 프로브를 물려받습니다. 어휘는 파이프라인을 실행할 때마다 성장합니다. 시간이 지나면 프로젝트의 도메인 모듈이 살아있는 지식 베이스가 되는 거죠 — 코드가 *하는 것*뿐 아니라 이 도메인에서 *중요한 것*이 무엇인지의 기록.

---

## 네 스킬의 조합: End-to-End 시나리오

네 스킬은 독립적으로도, 함께도 쓸 수 있습니다. 하지만 조합은 단순히 "이걸 실행하고, 저걸 실행하고"가 아닙니다 — 시나리오마다 고유한 리듬이 있고, 다른 아티팩트가 단계 사이를 흐르며, 사람이 개입하는 방식도 단계마다 달라집니다.
...
이 시나리오는 근본적으로 다릅니다 — 아무것도 새로 빌드하지 않습니다. 기존 코드를 설명하는 스펙, 플랜, 태스크 분해를 만들어서 유지보수와 확장을 가능하게 하는 거죠.
...
이 패턴 — 파이프라인에서 나와 조사하고, 다시 돌아오기 — 이 네 스킬이 느슨하게 결합된 이유입니다. 각각이 영구적인 파일 아티팩트를 만듭니다. 파이프라인은 에이전트 메모리가 아니라 파일을 읽습니다. 그래서 중단하고, 탐색하고, 상태를 잃지 않고 재개할 수 있습니다.
...
domain_extend:
  소스: 파이프라인 실행 중 발견, 기존 문서 임포트, 수동 생성
  출력: domains/에 새 concern, archetype, 또는 foundation 모듈
  검증: 스키마 준수, 섹션 번호 일관성, Cross-Concern Integration
  핵심 속성: 생성된 모듈은 내장 모듈과 동일하게 작동 (자동 발견, append 병합)
```

---

*이 글은 Claude Code를 사용하여 작성되었습니다. spec-kit-skills 프로젝트 전체가 인간-AI 협업으로 개발되었습니다.*

---

📖 **전체 레퍼런스가 필요하신가요?** [기술 레퍼런스 매뉴얼 (114페이지, PDF)](https://github.com/coolhero/spec-kit-skills/releases/download/v0.2.0/spec-kit-skills-technical-reference-ko.pdf)을 다운로드하세요 — 설계 철학부터 모듈 스키마, 실패 패턴까지 모두 다룹니다.
