...
**Repository**: [github.com/coolhero/spec-kit-skills](https://github.com/coolhero/spec-kit-skills)
...
1. **Remembers** across Features (Feature 3 knows Feature 1's data models)
2. **Adapts** to your project type (an Electron app gets different rules than a REST API)
3. **Enforces** quality gates (the agent can't skip verification just because the build passed)
4. **Documents** decisions in files, not in the agent's memory (which evaporates between sessions)
...
**spec-kit-skills** is a set of three Claude Code skills that wrap [spec-kit](https://github.com/github/spec-kit) (GitHub's Specification-Driven Development tool) with project-wide awareness:
...
```
  Understand          Specify           Build            Verify
┌───────────┐   ┌──────────────┐   ┌───────────┐   ┌───────────┐
...
1. **It's files, not memory**: Every decision, every registry, every spec lives in your repo. You can `git diff` the agent's work. You can edit the spec and re-run. You can switch sessions and nothing is lost.

2. **It enforces, not suggests**: HARD STOP gates require your explicit approval. The agent literally cannot proceed without your `AskUserQuestion` response. This isn't a "best practice" — it's a blocking gate.

3. **It adapts to your domain**: 48 concern modules, 15 archetypes, 40+ framework foundations. The verification rules for an Electron app are different from a Django server. This isn't a generic checklist.

4. **It composes**: Use code-explore alone to study a codebase. Use smart-sdd alone for greenfield. Use all three together for a full rebuild. Each skill works independently; together they form a pipeline.
...
skills:
  - name: code-explore
    ...
    purpose: Interactive source code exploration producing documented understanding
    commands: [orient, trace, synthesis, status]
    output_path: specs/explore/
    key_concept: Understand before you build

  - name: reverse-spec
    trigger: /reverse-spec
    purpose: Extract Global Evolution Layer from existing codebases
    output_path: specs/reverse-spec/
    key_concept: Source code → structured specs

  - name: smart-sdd
    trigger: /smart-sdd
    purpose: Orchestrate full SDD pipeline with cross-Feature memory
    commands: [init, add, pipeline, adopt, status, coverage, parity, expand, reset]
    output_path: specs/
    key_concept: Every Feature knows about every other Feature

core_concepts:
  global_evolution_layer:
    what: Cross-Feature memory stored in files
    artifacts: [entity-registry.md, api-registry.md, sdd-state.md, roadmap.md]
    principle: File over Memory — never store state in agent context alone
  ...
  domain_profile:
    ...
    principle: Adapt to project type, don't use generic checklists

  brief:
    what: Structured Feature intake (6-step consultation)
    steps: [scope, actors, entities, error_paths, dependencies, approval]
    principle: 2 minutes of structure saves 2 hours of rework

enforcement:
  hard_stops: Every checkpoint requires explicit user approval via AskUserQuestion
  pipeline_guards: 7 guards (G1-G7) that block pipeline progression
  empty_response: If user response is empty, re-ask (never proceed on silence)
...
scenarios:
  ...
  mid_pipeline_investigation: /code-explore --no-branch (safe during active pipeline)
```
