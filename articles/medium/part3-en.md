...
To see the difference this makes, consider the before and after. Before context injection, the agent received pre-context.md and a generic instruction to "generate a spec." The resulting spec was shallow — functional requirements without SC detail, no awareness of domain-specific patterns, no connection to source behavior. After the 7-stage injection protocol, the agent produces a spec that contains actual UI control types (dropdown, not text input), preserves source behavior mappings (SBI B-numbers linked to FR-numbers), includes cross-concern interaction SCs (the optimistic-update-plus-reconnection patterns), and references existing entities by their canonical registry names. The spec reads like it was written by someone who understands both the source application and the target architecture.
