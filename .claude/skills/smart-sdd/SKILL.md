...
- 선행 Feature의 실제 구현 결과 (`.specify/` 하위)가 있으면 함께 참조한다
...
- spec-kit 커맨드가 생성/수정하는 파일은 `.specify/` 하위에 위치한다
...
| `plan` | spec.md 존재 | `.specify/specs/[feature-name]/spec.md` 존재 확인 |
| `tasks` | plan.md 존재 | `.specify/specs/[feature-name]/plan.md` 존재 확인 |
| `implement` | tasks.md 존재 | `.specify/specs/[feature-name]/tasks.md` 존재 확인 |
...
- `.specify/` 디렉토리의 파일을 직접 수정하지 않는다. spec-kit 커맨드를 통해서만 변경한다.
