...
Execute the **full Common Protocol** (Assemble → Checkpoint → Execute → Review → Update):

1. **Assemble**: Read `BASE_PATH/constitution-seed.md`
...
2. **Checkpoint (HARD STOP)**: Display the constitution-seed content and provide the user an opportunity to revise/supplement. MUST use AskUserQuestion and WAIT for approval before proceeding.
3. **Execute**: Provide the constitution-seed content as context and execute `speckit-constitution`
4. **Review (HARD STOP)**: Read `.specify/memory/constitution.md` and display the full finalized content. MUST use AskUserQuestion with "Approve", "Request modifications", "Edit manually" and WAIT for explicit approval. **If the user's response is empty or blank, this is NOT approval — re-ask.** Constitution is the most critical artifact — it governs all subsequent Features. Do NOT skip this Review.
...
- **Origin must be `reverse-spec`**: Parity checking is only available for brownfield rebuild projects. If Origin in sdd-state.md is `greenfield` or `add`, display: "⚠️ Parity check is only available for brownfield rebuild projects (Origin: reverse-spec)." and exit.
