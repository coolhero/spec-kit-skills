...
| DOM measurement in async lifecycle | Lifecycle hook body containing `getBoundingClientRect`, `offsetHeight`, `scrollHeight` (React `useEffect`, Vue `onMounted`, Svelte `onMount`, Angular `ngAfterViewInit`) | ⚠️ warning — layout flicker |
| Missing error boundary | Route/page-level components without error boundary (React `ErrorBoundary`/`error.tsx`, Vue `errorCaptured`, Svelte `<svelte:boundary>`, Angular `ErrorHandler`) | ⚠️ warning |
...
- For each new store: grep the app entry point (e.g., React `App.tsx`/`_app.tsx`, Vue `main.ts`, Svelte `+layout.svelte`, Django `wsgi.py`/`asgi.py`, Go `main.go`, Flask `app.py`) for `hydrate()` or equivalent initialization call
