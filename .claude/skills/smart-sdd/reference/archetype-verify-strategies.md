...
| **desktop-app** | Launch via `_electron.launch()` (Electron) or native binary (Tauri). NOT a browser URL. Wait for main window | Window visible + IPC bridge responding (send test IPC message → receive response) | `window-auto` — Playwright Electron API for UI interaction; `ipc-auto` — send IPC message → verify response; `tray-auto` — verify tray icon menu items (user-assisted if OS-native) | Verify clean shutdown: app process exits, no orphan child processes, no lock file remnants |
