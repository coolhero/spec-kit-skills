...
| `features/[FID]/ui-flows.md` | absent (non-GUI Feature, or Runtime Exploration not performed) | Skip UI flow injection for plan/implement. Plan generates interaction flow from spec FR/SC instead. Note in Checkpoint: "⚠️ No ui-flows.md — plan/implement will derive interaction flow from spec only." |
