...
| `pre-context.md` UI Flow Specifications | absent (greenfield/add, or rebuild without Runtime Exploration) | Skip UI Flow Spec injection for specify/plan/implement/verify. Agent generates FR/SC from SBI and code analysis only. Note in Checkpoint: "⚠️ No UI Flow Specs — FR/SC will lack interaction detail (UI control types, auto-fill behavior, error paths). Consider running reverse-spec Phase 1.5 Runtime Exploration to improve spec quality." |
