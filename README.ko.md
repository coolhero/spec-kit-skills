...
현재 **애플리케이션 개발** (backend, frontend, fullstack, mobile, library)에 최적화되어 있습니다. **데이터 사이언스**, **AI/ML**, **임베디드 시스템** 등 전문 도메인 특화 분석 프로필을 확장할 계획입니다. 각 스킬의 `domains/` 디렉토리에 프로필 스키마(`_schema.md`)와 기존 프로필이 있습니다. `--domain`으로 프로필을 선택합니다 (기본값: `app`).
