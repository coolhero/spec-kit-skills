...
[English README](README.md) | [Playwright 설정 가이드](PLAYWRIGHT-GUIDE.md) | [Lessons Learned](lessons-learned.md) | Last updated: 2026-03-21 11:08 KST
...
이 프로젝트에 SDD 문서가 이미 있는가?
  │
  ├─ YES (Case A) ─────────────────────────────────────────────────────────┐
  │   Hotfix/Patch: /smart-sdd add → 경량 pipeline F00X                   │
  │   Minor/Major:  /smart-sdd add --gap → /smart-sdd pipeline F00X      │
  │   Platform:     /smart-sdd add --gap → 단계적 /smart-sdd pipeline    │
  │                                                                       │
  └─ NO (Case B) ──────────────────────────────────────────────────────────┐
      Hotfix: 타겟 스캔 (영향 컴포넌트 + 호출자) → 수정 → 기록           │
      Patch:  부분 adopt (영향 범위) → add → pipeline                     │
      Minor:  부분 adopt (영향 + 인접) → add → pipeline                   │
      Major:  /reverse-spec --adopt → /smart-sdd adopt → add → pipeline   │
      Platform: 전체 /reverse-spec → /smart-sdd adopt → add → pipeline    │
```
...
| `domains/foundations/chrome-extension.md` | Chrome Extension (Manifest V3) Foundation — F0 감지 스텁, 아키텍처 노트 (서비스 워커, 콘텐츠 스크립트, 메시지 패싱) |
| `domains/foundations/rust-cargo.md` | Rust (Cargo) Foundation — F0 감지 스텁, 아키텍처 노트 (크레이트 구조, 가시성, 트레이트, 피처 플래그) |
| `domains/foundations/svelte.md` | Svelte Foundation — F0 감지 스텁, 아키텍처 노트 (컴포넌트, 반응성, SvelteKit 라우팅) |
| `domains/foundations/spring-framework.md` | Spring Framework (non-Boot) Foundation — F0 감지 스텁, XML 설정, 수동 와이어링, WAR 배포 |
| `domains/foundations/cmake.md` | CMake (C/C++) Foundation — F0 감지 스텁, 타겟 기반 모듈, 전처리기 조건부 컴파일, Conan/vcpkg |
| `domains/foundations/makefile.md` | Makefile/Autotools/Meson/Bazel Foundation — F0 감지 스텁, 멀티 빌드 시스템 툴체인 커맨드 |
| `domains/foundations/python.md` | Python (Standalone) Foundation — F0 감지 스텁, virtualenv/pip/poetry/uv 툴체인 |
| `domains/foundations/go.md` | Go (Standalone) Foundation — F0 감지 스텁, go modules, 빌드 태그, CGO |
| `domains/foundations/swift-spm.md` | Swift (SPM/Xcode) Foundation — F0 감지 스텁, Package.swift, Xcode 프로젝트 구조 |
| `domains/foundations/erlang-otp.md` | Erlang/OTP Foundation — F0 감지 스텁, rebar3, OTP 애플리케이션 구조 |
| `domains/foundations/nuxt.md` | Nuxt Foundation — F0 감지 스텁, auto-imports, 서버 라우트, Nitro 엔진 |
| `domains/foundations/angular.md` | Angular Foundation — F0 감지 스텁, modules/standalone 컴포넌트, 의존성 주입 |
| `domains/foundations/remix.md` | Remix Foundation — F0 감지 스텁, loader/action 패턴, 중첩 라우팅 |
| `domains/foundations/qt.md` | Qt (C++) Foundation — F0 감지 스텁, QML, signals/slots, MOC |
| `domains/foundations/gtk.md` | GTK Foundation — F0 감지 스텁, GObject, 위젯 계층, Glade UI |
| `domains/foundations/symfony.md` | Symfony Foundation — F0 감지 스텁, 번들, 서비스 컨테이너, Doctrine |
| `domains/foundations/wordpress.md` | WordPress Foundation — F0 감지 스텁, hooks/filters, 테마/플러그인 아키텍처 |
| `domains/foundations/android-native.md` | Android Native (Kotlin/Java) Foundation — F0 감지 스텁, Gradle, activity/fragment 라이프사이클 |
| `domains/concerns/cqrs-eventsourcing.md` | CQRS/Event Sourcing concern — R1 감지, R3 Aggregate 기반 Feature 경계 규칙, R4 command→event→projection 흐름 |
| `domains/concerns/distributed-consensus.md` | Distributed consensus concern — R3 합의 프로토콜 추출, 리더 선출 패턴 |
| `domains/concerns/dag-orchestration.md` | DAG orchestration concern — R3 태스크 의존성 추출, 스케줄러 통합 패턴 |
| `domains/concerns/ecs.md` | ECS concern — R3 엔티티-컴포넌트-시스템 추출, 데이터 지향 설계 패턴 |
| `domains/concerns/wire-protocol.md` | Wire protocol concern — R3 프로토콜 메시지 추출, 프레이밍/직렬화 패턴 |
| `domains/concerns/k8s-operator.md` | K8s operator concern — R3 CRD/컨트롤러 추출, 재조정 루프 패턴 |
| **Contexts** | |
| `domains/contexts/migration.md` | Migration context — R3 마이그레이션 유형별 Feature 경계, R4 의존성/데이터 흐름 추출, R5 범위 추정 |
...
| `domains/concerns/hardware-io.md` | Hardware I/O — S0 + R1 (ioctl, mmap, serial, USB, GPIO, KVM, 디바이스 파일 접근) |
| `domains/concerns/cqrs-eventsourcing.md` | CQRS/Event Sourcing — S0 + R1 (Axon Framework, EventStoreDB, aggregate/command/event/query 핸들러 패턴) |
| `domains/concerns/distributed-consensus.md` | Distributed consensus — S0 + R1 (Raft, Paxos, gossip, 리더 선출, 쿼럼 패턴) |
| `domains/concerns/dag-orchestration.md` | DAG orchestration — S0 + R1 (Airflow, Prefect, Dagster, dbt, 태스크 의존성 패턴) |
| `domains/concerns/ecs.md` | ECS — S0 + R1 (엔티티-컴포넌트-시스템, 데이터 지향 설계, archetype 스토리지) |
| `domains/concerns/wire-protocol.md` | Wire protocol — S0 + R1 (MQTT, AMQP, RESP, WebRTC, 바이너리 프레이밍 패턴) |
| `domains/concerns/k8s-operator.md` | K8s operator — S0 + R1 (CRD, controller-runtime, 재조정 루프, finalizer 패턴) |
| **Archetypes** | |
...
| `domains/archetypes/database-engine.md` | Database engine — A0 시맨틱 + 코드 패턴 (스토리지 엔진, 쿼리 프로세서, B-tree/LSM) |
| `domains/archetypes/network-server.md` | Network server — A0 시맨틱 + 코드 패턴 (프록시, 로드 밸런서, L4/L7 트래픽) |
| `domains/archetypes/message-broker.md` | Message broker — A0 시맨틱 + 코드 패턴 (Kafka, RabbitMQ, NATS, 이벤트 스트리밍) |
| `domains/archetypes/game-engine.md` | Game engine — A0 시맨틱 + 코드 패턴 (Unity, Unreal, Godot, ECS, 렌더 루프) |
| `domains/archetypes/browser-extension.md` | Browser extension — A0 시맨틱 + 코드 패턴 (매니페스트, 콘텐츠 스크립트, 백그라운드 워커) |
| `domains/archetypes/infra-tool.md` | Infra tool — A0 시맨틱 + 코드 패턴 (Terraform, Pulumi, 재조정 엔진) |
| **Contexts** | |
| `domains/contexts/migration.md` | Migration context — M0 시그널 감지, M1 규모 (Hotfix→Platform), M2 대상 계층, M3 영향 평가, M4 파이프라인 깊이 조절 |
| **런타임** | |
| `runtime/_index.md` | 런타임 모듈 인덱스 — Domain Profile 연동, Interface별 동작 차이 |
| `runtime/playwright-detection.md` | Playwright 가용성 감지 — CLI/MCP/CDP 프로브, Electron 연결 모드 |
| `runtime/data-storage-map.md` | 데이터 저장소 감지 — 설정 저장소, DB, 파일, lock 분석, userData 경로 |
| `runtime/user-assisted-setup.md` | 사용자 설정 흐름 — BLOCKING/PARTIAL/OPTIONAL 분류, 앱 유형별 안내 |
| `runtime/app-launch.md` | 앱 실행 + Playwright 연결 — Electron/Web/API/CLI + userData 공유 |
| `runtime/observation-protocol.md` | 3-layer 관찰 프레임워크 — Common (화면, a11y 트리) + Domain-Aware (축별 관찰) + Skill-Specific (폼 필드, SC 증거) |
| **Reference** | |
| `reference/completion-report.md` | 공유 Completion Analysis Report 템플릿 — reverse-spec (Phase 4)과 smart-sdd adopt (Post-Pipeline)에서 사용, 모드별 전용 섹션 포함 |
