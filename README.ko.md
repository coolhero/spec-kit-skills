...
의존성 그래프를 구성한 후 **Tier별로 그룹화(Tier 1 → Tier 2 → Tier 3)한 뒤 각 Tier 내에서 위상정렬 순서로 Feature ID(F001, F002, ...)를 배정**하여, Tier 그룹이 유지되면서도 구현 가능 순서를 반영합니다. 각 Feature를 **5가지 분석 축**으로 종합 평가합니다:
