...
### `/reverse-spec`이 필요한 이유

`/reverse-spec` 스킬은 단순히 문서화 도구가 아닙니다. **smart-sdd가 제대로 작동하기 위한 필수 전제조건**을 생성합니다.

기존 소스코드를 역분석하여 엔티티, API 계약, 비즈니스 로직, Feature 간 의존성을 추출하는 이유는, 이 정보가 있어야만 smart-sdd가 각 Feature의 spec-kit 커맨드 실행 시 **교차 Feature 컨텍스트를 정확하게 주입**할 수 있기 때문입니다. 역분석 없이는 smart-sdd가 어떤 엔티티를 참조해야 하는지, 어떤 API 계약을 준수해야 하는지, 선행 Feature에 어떻게 의존하는지 알 수 없습니다.

또한 기존 구현을 **재현하여 테스트**하는 것이 핵심입니다. 추출된 요구사항 초안(FR-###)과 수용 기준(SC-###)은 기존 시스템이 실제로 무엇을 하는지를 기반으로 만들어지므로, 재개발된 시스템이 기존 기능을 정확히 재현하는지 검증할 수 있는 테스트 기준이 됩니다.
...
| **Checkpoint** | 조립된 컨텍스트를 요약 형태로 사용자에게 보여주고 승인/수정 기회 제공. 사용자가 수정을 요청하면 반영 후 재확인. **`--auto` 모드에서만 생략** (요약은 표시하되 즉시 실행 진행). `--dangerously-skip-permissions` 환경에서는 AskUserQuestion 대신 일반 텍스트 메시지로 확인 요청 |
