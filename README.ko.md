...
[English README](README.md) | [Playwright 설정 가이드](PLAYWRIGHT-GUIDE.md) | [Lessons Learned](lessons-learned.md) | Last updated: 2026-03-16 09:18 KST
...
3. **합성 가능한 도메인 지식** — 프로젝트 유형은 크게 다릅니다(REST API vs 데스크톱 앱 vs AI 어시스턴트). 하나의 거대한 규칙 세트 대신, 도메인 지식을 4개 축(Interface × Concern × Archetype × Scenario)과 Foundation 레이어로 분해하여 자유롭게 합성합니다. 데스크톱 AI 재구축은 `gui + ipc + async-state + ai-assistant + rebuild + electron`을 로드하고, 그린필드 퍼블릭 API는 `http-api + auth + public-api + greenfield + express`를 로드합니다. 불필요한 규칙 없고, 누락된 컨텍스트 없습니다.
...
도메인 동작(SC 생성, 검증, 프로브, 버그 방지)은 프로젝트 유형마다 다릅니다. REST API는 엔드포인트 상태 코드 검사가 필요하고, 데스크톱 앱은 IPC 경계 안전성 검사가 필요하며, 재구축 프로젝트는 동작 패리티 검증이 필요하고, AI 어시스턴트는 스트리밍 우선 원칙이 필요합니다. 모든 프로젝트에 모든 규칙을 로드하는 단일 파일 대신, 도메인 지식을 **네 개의 독립 축**으로 분해합니다:
...
단계별 확장 가이드(인터페이스, 관심사, 아키타입, Foundation, 프로필, 시나리오 추가)와 5단계 정교화 모델의 상세 내용은 [ARCHITECTURE-EXTENSIBILITY.md](ARCHITECTURE-EXTENSIBILITY.md) 참고.
...
각 도메인 모듈은 **S0 신호 키워드**(인터페이스/관심사) 또는 **A0 신호 키워드**(아키타입)를 선언합니다 — 해당 모듈이 활성화되어야 함을 나타내는 용어들입니다. 아이디어 문자열로 프로젝트를 시작하면 (`init "AI로 웹 페이지를 요약하는 Chrome 확장 프로그램"`), 에이전트가 모든 S0 및 A0 키워드를 스캔하여 Domain Profile과 Archetype을 자동으로 추론합니다. "React"는 `gui`를, "REST API"는 `http-api`를, "OpenAI"는 `external-sdk` 관심사와 `ai-assistant` 아키타입을 동시에 트리거합니다 — 수동 설정 없이 자동으로.
...
Foundation 파일은 이제 선택적 **F7 Philosophy** 섹션을 포함합니다. 이 섹션은 프레임워크가 권장하는 원칙(예: Electron의 "Process Crash Isolation", Express의 "Middleware Composition")을 캡처합니다. F7은 F0–F6 운영 체크리스트와 구별됩니다 — _무엇을_ 설정할지가 아니라 _왜_ 특정 패턴이 선호되는지를 정의합니다.
