...
| **F** (F0–F8) | reverse-spec / smart-sdd | **프레임워크별** 인프라 결정 | 플랫폼 청사진 |
...
| **F8** | Toolchain | `implement` / `verify` | **빌드/테스트/린트 명령** — `npm run build`, `cargo test` |
...
| 결정 이력 | `history.md` |
| 실패 패턴 & 대응책 | [`lessons-learned.md`](lessons-learned.md) — 20개 갭 패턴 + 70개 구체적 교훈. AI 에이전트 파이프라인을 설계하는 누구에게나 유용합니다. |
