...
[English README](README.md) | [Playwright 설정 가이드](PLAYWRIGHT-GUIDE.md) | [Lessons Learned](lessons-learned.md) | Last updated: 2026-03-17 13:50 KST
...
**2. 에이전트에게 프로젝트 유형 인식이 없다.** REST API 프로젝트와 Electron 데스크톱 앱은 근본적으로 다른 규칙이 필요합니다 — 다른 테스트 전략, 다른 검증 체크, 다른 버그 패턴. 하지만 에이전트는 모든 프로젝트를 같은 방식으로 처리하며, 범용적인 spec을 생성하고 도메인 특화 실패 모드를 놓칩니다.

**3. 에이전트는 받은 입력을 그대로 수용한다.** Feature가 상세한 PRD에서 시작하든, 캐주얼한 대화에서 시작하든, 코드 gap 분석에서 시작하든, 에이전트는 받은 설명을 그대로 받아들이고 코딩을 시작합니다. Feature 정의가 완전한지 확인하는 구조화된 프로세스가 없습니다 — 누락된 에지 케이스, 미정의 데이터 모델, 불명확한 경계는 구현 중에야 드러나며, 그때 고치는 비용은 훨씬 큽니다.

### spec-kit이 하는 일

[spec-kit](https://github.com/github/spec-kit)은 첫 번째 층을 해결합니다: **Specification-Driven Development (SDD)** — 프로젝트를 Feature로 나누고, 각각에 spec을 쓰고, 그에 맞춰 코딩합니다. 에이전트에게 구조화된 파이프라인(specify → plan → implement → verify)을 줍니다.
