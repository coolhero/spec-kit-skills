...
분석 결과를 기반으로 논리적 기능 단위(Feature)를 식별합니다. 의존성 그래프를 구성한 후 **Tier별로 그룹화(Tier 1 → Tier 2 → Tier 3)한 뒤 각 Tier 내에서 위상정렬 순서로 Feature ID(F001, F002, ...)를 배정**하여, Tier 그룹이 유지되면서도 구현 가능 순서를 반영합니다. 각 Feature를 **5가지 분석 축**으로 종합 평가합니다:
...
| **VI. Demo-Ready Delivery** | 각 Feature 완료 시 데모 가능한 형태로 제공. "테스트 통과"만으로는 불충분 — 최소한의 데모 표면(CLI 명령, 데모 스크립트, 데모 페이지)을 구현하고 `demos/F00N-name.md`에 문서화 | 비개발자도 `demos/F00N-name.md`를 따라 Feature 동작을 확인 가능 |
