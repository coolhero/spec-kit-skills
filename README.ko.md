...
[English README](README.md) | [Playwright 설정 가이드](PLAYWRIGHT-GUIDE.md) | [Lessons Learned](lessons-learned.md) | 📖 [기술 레퍼런스 매뉴얼 (PDF)](https://github.com/coolhero/spec-kit-skills/releases/download/v0.2.0/spec-kit-skills-technical-reference-ko.pdf) | Last updated: 2026-03-26 18:43 KST
...
- [해결하는 문제](#해결하는-문제)
- [설계 규율](#설계-규율)
- [스킬](#스킬) — code-explore, reverse-spec, smart-sdd, domain-extend
- [시나리오 가이드](#시나리오-가이드) — S1~S9 사용 시나리오
- [사용자 여정](#사용자-여정) — end-to-end 워크스루
- [빠른 예시](#빠른-예시)
- [아키텍처](#아키텍처) — 3가지 설계 원칙, 파이프라인 가드
- [도메인 모듈 시스템](#도메인-모듈-시스템) — 5축 모델, 모듈 섹션
- [확장성 & 커스터마이징](#확장성--커스터마이징)
- [세션 복원력 & 에이전트 거버넌스](#세션-복원력--에이전트-거버넌스)
- [상세 레퍼런스](#상세-레퍼런스) — 공통 프로토콜, 스킬별 워크플로우
- [레퍼런스](#레퍼런스) — 설치, 경로, 네이밍
- [파일 맵](#파일-맵)
...
기존 코드가 있는가?
  ...
  있음 → 목표는?
          ...
          재구축 (동일)  → /reverse-spec → init --from-reverse-spec 또는 pipeline  (S4: 리빌드)
...
커스텀 도메인 모듈이 필요한가 (프레임워크/패턴이 커버되지 않는 경우)?
...
/domain-extend browse    # 도메인 모듈 시스템 개요 표시
```
...
Domain Profile은 규칙을 생산하고 깊이를 조절하는 **5개 축**으로 구성됩니다:
...
| 축 4 | **Foundation** | 프레임워크별 제약과 도구 체인 | React, Electron, Next.js (40+ 프레임워크) |
| 축 5 | **Context** | 프로젝트 상황 (모드 + 스케일 + 수정자) | greenfield, rebuild, adoption + production×small-team + migration |

각 축이 규칙(SC 품질 기준, 버그 방지 패턴, 검증 전략)을 기여합니다. Context 축은 세 가지 컴포넌트로 구성됩니다: **Mode**(어떤 파이프라인 라이프사이클 — greenfield, rebuild, incremental, adoption), **Scale**(적용 강도 — project_maturity × team_context, 예: prototype×solo vs production×large-team), **Modifiers**(특수 상황 — +migration, +compliance 등). prototype은 기능적 SC만 요구하고 테스트는 선택이지만, production은 엣지 케이스까지 완전한 커버리지와 필수 observability를 요구합니다.
...
`init`이나 `add`를 실행하면 시스템이 프로젝트의 **Domain Profile Instance** — YOUR 프로젝트에 적용되는 구체적인 축 조합(예: `gui + auth + realtime + electron + rebuild`)을 추론합니다. 이 Instance는 `sdd-state.md`에 저장되고, 상세한 도메인 결정은 `domain-profile-instance.md`에 기록됩니다. Instance가 각 파이프라인 단계에서 어떤 모듈 규칙이 로드되는지를 결정합니다. Domain Profile은 질문지 템플릿이고, Instance는 작성 완료된 답변지입니다.

Domain Profile은 **first-class citizen**입니다 — 한번 설정하고 잊는 구성이 아니라, 모든 스킬의 모든 단계에서 적극적으로 동작하는 살아 있는 컨텍스트입니다:

- **code-explore**: orientation에서 소스 프로젝트의 프로파일(5개 축)을 감지하고, 탐색할 흐름을 가이드하며, synthesis에서 사용자의 타겟 프로파일을 도출합니다
...
- **specify → plan → implement → verify**: 각 단계가 프로파일 고유 규칙을 로드하고 Context scale이 필터링합니다 — production 데스크톱 앱의 IPC는 프로세스 경계 안전 검사가 필수이지만, MVP 마이크로서비스의 dead-letter 처리는 권장(비차단) 패턴이 됩니다
...
**세 개념이 서로를 강화하는 방식**: Domain Profile은 Brief에 도메인 특화 질문을 주입합니다 — 프로젝트에 `auth` concern이 있으면 Brief가 자동으로 "인증 전략은? JWT, 세션, OAuth?" 하고 묻습니다. `realtime`이 활성화되어 있으면 "연결이 끊기면 어떻게 되나요?" 라고 묻죠. 이 질문들은 도메인 모듈의 S5 (Elaboration Probes)에서 나옵니다. Domain Profile이 풍부할수록 Brief의 질문이 깊어지는 거죠. 그리고 파이프라인이 기존 모듈로 커버 안 되는 갭을 발견하면, `/domain-extend`로 새 모듈을 만들고 — 그 모듈의 S5 질문이 *다음* Brief를 더 똑똑하게 만듭니다. GEL이 기억하고, Domain Profile이 규칙을 정하고, Brief가 검증하고 — domain-extend가 이 세 개를 움직이는 어휘를 키웁니다.
...
### `/domain-extend` — 도메인 모듈 시스템 커스터마이즈

5축 도메인 모듈 시스템을 탐색하고 확장하고 커스터마이즈하는 인터랙티브 도구입니다. 기존 모듈이 프로젝트 패턴을 커버하지 않거나, 내부 문서를 규칙으로 변환하고 싶거나, 조직 전역 convention이 필요할 때 사용합니다.

```bash
/domain-extend browse                        # 시스템 개요: 모든 축, 모듈 수
/domain-extend browse concerns               # 전체 Concern 모듈 목록
/domain-extend detect                        # sdd-state.md 기반 프로필 gap 분석
/domain-extend detect /path/to/code          # 코드베이스에서 미커버 패턴 스캔
/domain-extend detect --from-explore ./specs # explore 아티펙트와 모듈 교차 참조
/domain-extend extend concerns/rate-limiting # 새 Concern 모듈 생성
/domain-extend import /docs/style-guide.md   # 내부 문서를 모듈 섹션으로 변환
/domain-extend customize auth --org          # 조직 수준 auth convention 오버레이 생성
/domain-extend validate                      # 전체 모듈 스키마 준수 검증
```

**여섯 가지 명령**: `browse` (탐색), `detect` (gap 발견), `extend` (새 모듈 생성), `import` (문서 → 모듈 변환), `customize` (조직/프로젝트 오버레이), `validate` (스키마 검증)

**연동**: `detect`는 code-explore 아티펙트(`--from-explore`)와 reverse-spec 분석을 사용하여 기존 모듈이 커버하지 않는 패턴을 식별합니다. `extend`나 `import`로 생성한 모듈은 smart-sdd와 reverse-spec 파이프라인에서 즉시 사용 가능합니다. 모듈 파일을 직접 편집할 수도 있습니다 — domain-extend는 편의 도구이지 gatekeeper가 아닙니다.
...
다이어그램은 전체 생명주기를 보여줍니다: code-explore로 기존 코드를 **이해**하고, reverse-spec으로 **분석**(소스 앱 실행 + Feature별 spec-draft 생성)하거나 init으로 새로 시작하고, Brief 프로세스를 통해 Feature를 **정의**한 다음, spec-kit 파이프라인을 통해 **구축**(specify는 spec-draft를 보완, 처음부터 생성하지 않음)합니다. `/domain-extend`는 선택적 확장 도구로 — code-explore나 reverse-spec 분석에서 gap을 감지하고, 새 모듈을 생성하여 모든 파이프라인 단계를 형성하는 Domain Profile에 공급합니다. 프로젝트 전체 아티펙트는 `specs/_global/`에, 각 Feature의 모든 아티펙트(분석 + 파이프라인)는 `specs/NNN-feature/`에 — 깨끗한 구조.
...
/reverse-spec ./old-source
  → /smart-sdd init --from-reverse-spec specs/reverse-spec/   (리뷰 체크포인트 포함)
  또는: /smart-sdd pipeline --all                               (리뷰 없이 바로 진행)
  → /smart-sdd parity
...
이 값들은 `sdd-state.md`에 저장되며 관련 파이프라인 단계에서 자동으로 읽힙니다 — 전체 소비 매트릭스는 `domains/contexts/modes/rebuild.md` 참고.
...
프로젝트마다 필요한 검사 규칙이 다릅니다 — REST API는 상태 코드 검사, 데스크톱 앱은 IPC 안전성 검사, AI 앱은 스트리밍 검증이 필요합니다. 모든 규칙을 하나의 파일에 넣는 대신, **5개 축**으로 분해하여 프로젝트에 맞는 조합만 로드합니다:
...
├── gui                 ├── audit-logging             ├── browser-extension   ├── nextjs
├── cli                 ├── auth                      ├── cache-server        ├── express
├── data-io             ├── authorization             ├── compiler            ├── django
├── tui                 ├── codegen                   ├── database-engine     ├── spring-boot
├── mobile              ├── compliance                ├── game-engine         └── ... (40+)
├── library             ├── connection-pool           ├── infra-tool
├── embedded            ├── content-moderation        ├── inference-server   Axis 5: Context
├── grpc                ├── cqrs-eventsourcing        ├── media-server       (프로젝트 상황)
└── k8s-api             ├── cryptography              ├── message-broker     Mode:
                        ├── dag-orchestration         ├── microservice       ├── greenfield
                        ├── distributed-consensus     ├── network-server     ├── rebuild
                        ├── ecs                       ├── public-api         ├── incremental
                        ├── external-sdk              ├── sdk-framework      └── adoption
                        ├── geospatial                └── workflow-engine    Scale:
                        ├── gpu-compute                                      ├── prototype / mvp
                        ├── graceful-lifecycle                               │   / production
                        ├── hardware-io                                      └── × solo / small-team
                        ├── i18n                                                 / large-team
                        ├── infra-as-code
                        ├── iot-protocol
                        ├── ipc
...
- **Context** (Axis 5): 프로젝트의 _상황_은 무엇인가? Mode(greenfield/rebuild/adoption) + Scale(prototype~production × solo~large-team) + Modifiers(+migration 등)

**Domain Profile** = 5축 (Interface + Concern + Archetype + Foundation + Context). 예: `[gui] + [async-state, ipc] + ai-assistant + electron + rebuild` at `mvp × solo`. 에이전트는 프로젝트에 관련된 모듈만 로드합니다 — API 전용 프로젝트에 GUI 테스트 규칙이 로드되지 않고, AI 프로젝트에는 CRUD 앱에 불필요한 스트리밍 검증이 포함되며, prototype은 production 프로젝트에 필요한 observability 규칙이 적용되지 않습니다.
...
각 모듈은 단순히 "이 프로젝트는 auth를 사용한다"는 태그가 아닙니다. **번호가 매겨진 섹션**을 포함하는 파일이며, 각 섹션은 특정 파이프라인 단계에 입력됩니다.

**왜 네 가지 섹션 패밀리인가?** 각 패밀리는 다른 스킬과 다른 *목적*을 가집니다:

| 패밀리 | 스킬 | 목적 | 비유 |
|--------|------|------|------|
| **S** (S0–S9) | smart-sdd | 이 도메인을 어떻게 **빌드**하는가 | 시공 매뉴얼 |
| **A** (A0–A5) | smart-sdd | *왜* 특정 결정이 **중요한가** | 건축 철학 |
| **R** (R1–R5) | reverse-spec | 기존 코드에서 이 도메인을 어떻게 **읽는가** | 독해 가이드 |
| **F** (F0–F8b) | reverse-spec / smart-sdd | **프레임워크별** 인프라 결정 | 플랫폼 청사진 |

S-section은 파이프라인에 *무엇을 해야 하는지* (SC 생성, 버그 예방, 검증) 알려줍니다. A-section은 *왜* 해야 하는지 (streaming-first 철학이 모든 SC를 가이드) 알려줍니다. R-section은 reverse-spec에게 기존 코드에서 패턴을 *어떻게 인식*하는지 알려줍니다. F-section은 비즈니스 Feature가 위에 올라갈 *플랫폼*을 정의합니다.

**각 패밀리 내부 설계 철학**: 각 번호 섹션은 사용자 여정의 정확히 하나의 단계에 대응합니다 — 프로젝트 설정부터 Feature 정의, 명세, 구현, 검증까지. 이 분리의 장점: (1) **lazy loading** — 각 파이프라인 단계가 필요한 섹션만 로드 (specify는 S0/S1/S5/S9, implement는 S7/S6 — 모듈 컨텐츠의 55-70% 절감), (2) **독립적 확장** — `/domain-extend`로 새 모듈 생성 시 S5(질문)를 몰라도 S7(버그 패턴)을 채울 수 있고, 실패 모드를 발견하면 나중에 S7을 추가

**S-sections** (smart-sdd — 파이프라인 실행):

| 섹션 | 이름 | 파이프라인 단계 | 하는 일 |
|------|------|---------------|--------|
| **S0** | Signal Keywords | `init` / `detect` | 이 모듈을 **언제 활성화**하나 — 키워드 매칭 |
| **S1** | SC Generation Rules | `specify` | 어떤 **시나리오를 테스트**하나 — "auth 토큰 만료 SC 필수" |
| **S3** | Verify Steps | `verify` | 어떤 **추가 검사**를 하나 — 재접속 테스트, 부하 테스트 |
| **S4** | Data Integrity | 모든 단계 | **불변 규칙** — 저장 데이터 변환 금지, 표시 시점만 변환 |
| **S5** | Elaboration Probes | `add` (Brief) | 사용자에게 **뭘 물어보나** — "인증 전략? JWT, 세션, OAuth?" |
| **S6** | UI Testing | `implement` (gui) | UI를 **어떻게 테스트**하나 — Playwright, 스크린샷 비교 |
| **S7** | Bug Prevention | `implement` / `verify` | **흔한 실수** 예방 — "캐시 삭제 후 만료 토큰 수용" |
| **S8** | Runtime Verification | `verify` | **런타임에서 어떻게 확인**하나 — 앱 실행 → API 호출 → 응답 확인 |
| **S9** | Brief Completion | `add` (Brief) | **Brief가 완전한지** 판단 — "프로토콜 선택 명시" ✓ |

**A-sections** (아키타입 — 도메인 철학): 아키타입은 *왜* 특정 결정이 중요한지를 답합니다. S-section이 *무엇*을 확인하는지 설명한다면, A-section은 그 확인을 이끄는 *가치*를 설명합니다.

| 섹션 | 이름 | 파이프라인 단계 | 하는 일 |
|------|------|---------------|--------|
| **A0** | Signal Keywords | `init` / `detect` | 이 아키타입을 **언제 활성화**하나 |
| **A1** | Philosophy Principles | 모든 단계 | **핵심 가치** — "Streaming-First", "Model Agnosticism" |
| **A2** | SC Extensions | `specify` | 아키타입별 **시나리오** — concern이 제공하는 것 이상 |
| **A3** | Elaboration Probes | `add` (Brief) | 도메인별 **질문** — "단일 vs 다중 LLM 프로바이더?" |
| **A4** | Constitution Injection | `constitution` | 프로젝트 기반에 **원칙 포함** |
| **A5** | Brief Completion | `add` (Brief) | 아키타입별 **Brief 완전성** 판단 |

**R-sections** (reverse-spec — 소스 분석): reverse-spec에게 이 모듈 유형의 기존 코드를 *어떻게 읽는지* 알려줍니다. 소스 분석의 "독해 가이드"입니다.

| 섹션 | 이름 | 파이프라인 단계 | 하는 일 |
|------|------|---------------|--------|
| **R1** | Code Patterns | `analyze` Phase 1 | 이 모듈을 나타내는 **코드 패턴** — import, 데코레이터, 설정 |
| **R3** | Analysis Axes | `analyze` Phase 2 | **무엇을 추출**하나 — Feature 경계, 데이터 흐름 |
| **R4** | Data Flow | `analyze` Phase 2 | 이 도메인에서 **데이터가 어떻게 흐르는가** |
| **R5** | Feature Boundaries | `analyze` Phase 3 | 감지된 패턴을 Feature로 **어떻게 분할**하나 |

**F-sections** (Foundation — 프레임워크 인프라): 비즈니스 Feature 전에 결정해야 할 프레임워크별 인프라 사항. Feature가 위에 올라갈 "플랫폼"입니다.

| 섹션 | 이름 | 파이프라인 단계 | 하는 일 |
|------|------|---------------|--------|
| **F0** | Detection Signals | `analyze` Phase 1 / `init` | 프레임워크 **식별** — package.json, 설정 파일 |
| **F1** | Categories | `init` | **인프라 카테고리** — BST, SEC, MID, API 등 |
| **F2** | Decision Items | `init` / `analyze` | **결정할 사항** — "CORS 정책? 인증 전략? ORM?" |
| **F3** | Extraction Rules | `analyze` | 코드에서 기존 결정을 **어떻게 추출**하나 |
| **F7** | Philosophy | `constitution` | **프레임워크 가치** — "미들웨어 합성", "설정보다 규약" |
| **F8/F8b** | Toolchain + Runtime Environment | `implement` / `verify` | **빌드/테스트/린트 명령** (F8) + **서버 시작, 헬스 체크, 환경 변수 로딩** (F8b). 데모 스크립트, verify, implement 스모크가 F8b를 읽음 |
...
**프로젝트 로컬 모듈**: `/domain-extend`로 커스텀 모듈을 생성하면, 스킬 설치 디렉토리가 아닌 프로젝트 내부 `specs/domains/`에 저장됩니다. 커스텀 모듈이 프로젝트와 함께 버전 관리되며, 다른 프로젝트에 영향을 주지 않습니다. Resolver는 3단계 순서로 스캔합니다: **built-in** (스킬 설치 디렉토리) → **project** (`specs/domains/`) → **org** (`org-convention.md`). `--skill` 플래그를 사용하면 공유 spec-kit-skills 설치에 모듈을 기여할 수 있습니다. 프로젝트 로컬 모듈은 단일 파일 형식(모든 S/A/R/F 섹션을 하나의 파일에 포함)을 사용하며, 이는 shared/reverse-spec/smart-sdd 디렉토리에 분산된 built-in 모듈의 triple-structure와 다릅니다.
...
Profile (desktop-app, web-api, fullstack-web, cli-tool, ml-platform, sdk-library)
   │
   ├── Interface 모듈 (gui, http-api, cli, data-io, tui, mobile, library, embedded, grpc, k8s-api)
   ├── Concern 모듈 (47: auth, async-state, codegen, ipc, i18n, infra-as-code, ...)
...
**Context Reset Protocol** — 파이프라인 세션이 복구 불가능한 상태(반복 tool 실패, 순환 루프, 손상된 상태)에 빠지면, `pipeline.md`에 정의된 구조화된 리셋 시퀀스를 따릅니다: 현재 단계 중단, 진행 상태를 `sdd-state.md`에 저장, 마지막 안정 체크포인트에서 재개. 긴 세션의 작업 손실을 방지합니다.
...
Phase 5:  Integration Demo (조건부 — Demo Group의 모든 Feature가 verify 통과 시)
```
...
6단계 파이프라인 완료 후 확인: 구조적 패리티 → 로직 패리티 → 갭 리포트 → 개선 계획 → 완료 리포트
...
| 결정 이력 | `history.md` |
| 실패 패턴 & 대응책 | [`lessons-learned.md`](lessons-learned.md) — 20개 갭 패턴 + 86개 구체적 교훈. AI 에이전트 파이프라인을 설계하는 누구에게나 유용합니다. |
...
4개 스킬 + 1개 공유 모듈. 전체 파일 인벤토리는 [FILE-MAP.md](FILE-MAP.md) 참조.
...
│
├── {skill}/                       스킬별 디렉토리 (code-explore, reverse-spec, smart-sdd, domain-extend)
...
│   │   ├── contexts/              프로젝트 컨텍스트 규칙 — 모드 + 수정자 (smart-sdd 전용)
