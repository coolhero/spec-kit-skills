...
[English README](README.md) | Last updated: 2026-03-07 21:03 KST
...
요구사항은 변합니다. smart-sdd는 파이프라인 실행 도중 spec 아티팩트를 수정할 수 있는 여러 메커니즘을 제공합니다:

**1. Restructure — Feature 정의 변경**

`/smart-sdd restructure`로 Feature의 구조적 변경을 처리합니다:
- **Split**: Feature 하나 → 둘 이상으로 분할 (범위 축소)
- **Merge**: 둘 이상의 Feature → 하나로 병합 (관련 작업 통합)
- **Move**: Feature 간 요구사항, 엔티티, API 이동
- **Reorder**: 의존성 관계 변경
- **Delete**: Feature 삭제

모든 변경사항은 관련 아티팩트(roadmap, 레지스트리, pre-context, sdd-state)에 자동 전파되며, 실행 전 사용자 승인을 받습니다. 영향받는 단계는 🔀로 표시되어 재실행됩니다.

**2. Review 체크포인트에서 직접 편집**

모든 파이프라인 단계는 Review 체크포인트로 끝나며, 세 가지 옵션을 제공합니다:
- **"Approve"** — 다음 단계로 진행
- **"Request modifications"** — 에이전트에게 특정 변경 요청
- **"I've finished editing"** — spec 아티팩트를 직접 편집기에서 수정 후 복귀

즉, 어떤 Review 체크포인트(specify, plan, tasks, analyze, implement, verify)에서든 `spec.md`, `plan.md`, `tasks.md` 등을 편집기에서 열어 수정하면, 파이프라인이 수정된 아티팩트를 다시 읽고 검증한 후 진행합니다.

**3. Pipeline `--start` — 특정 단계부터 재실행**

변경 후 `/smart-sdd pipeline --start <step>`으로 특정 단계부터 파이프라인을 재실행할 수 있습니다 (예: `--start plan`으로 업데이트된 spec 기반 계획 재생성). 선행 조건은 자동 검증됩니다.

**4. Coverage / Parity — 완료 후 갭 탐지**

Feature 구현 완료 후:
- `/smart-sdd coverage` — 매핑되지 않은 소스 동작을 탐지하고 기존 Feature에 추가 가능 (specify부터 🔀 표시)
- `/smart-sdd parity` (브라운필드 리빌드) — 구조적·로직 갭을 탐지, 기존 Feature에 추가하거나 새 보완 Feature 생성 가능

**타이밍별 권장 방법**:

| 시점 | 권장 방법 |
|------|----------|
| specify/plan/tasks 중 (구현 전) | Review 체크포인트에서 편집 또는 `restructure` 사용 |
| implement/verify 중 (코드 존재) | 현재 단계 완료 후 `restructure` 또는 `--start` 사용 |
| 파이프라인 완료 후 | `coverage` 또는 `parity`로 갭 탐지 및 보완 |
